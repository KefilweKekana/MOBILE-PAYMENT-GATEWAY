...
frappe.ui.form.on("Sales Invoice", {
  refresh: function (frm) {
    if (frm.doc.docstatus === 1 && flt(frm.doc.outstanding_amount) > 0) {
      ...
      // "Send Payment SMS" button — sends both WaafiPay + Edahab links in one SMS
      if (frm.doc.waafi_payment_link || frm.doc.edahab_payment_link) {
        frm.add_custom_button(
          __("Send Payment SMS"),
          function () {
            frappe.call({
              method: "mobile_payments.utils.notifications.send_payment_sms_for_invoice",
              args: { invoice_name: frm.doc.name },
              freeze: true,
              ...
              callback: function (r) {
                if (r.message && r.message.sent) {
                  frappe.show_alert({
                    message: r.message.message || __("SMS sent successfully"),
                    indicator: "green",
                  }, 5);
                } else {
                  frappe.show_alert({
                    message: (r.message && r.message.message) || __("SMS could not be sent"),
                    indicator: "orange",
                  }, 5);
                }
              },
