...
var mobile_payments = {
  ...
  _initiate_hpp_payment: function (frm, provider, method) {
    let api_method =
      provider === "Edahab"
        ? "mobile_payments.api.edahab.initiate_edahab_hpp"
        : "mobile_payments.api.waafipay.initiate_waafipay_hpp";
    ...
    frappe.call({
      ...
      callback: function (r) {
        if (r.message && r.message.success) {
          let hpp_url = r.message.hpp_url;

          // Show HPP dialog with options
          let d = new frappe.ui.Dialog({
            title: __("Hosted Payment Page"),
            fields: [
              {
                fieldtype: "HTML",
                fieldname: "hpp_info",
                options: `
                  <div style="text-align:center; padding:20px;">
                    <i class="fa fa-external-link fa-3x" style="color:#3498db; margin-bottom:15px;"></i>
                    <h4>${__("Payment Page Ready")}</h4>
                    <p>${__("Click the button below to open the payment page, or share the link with the customer.")}</p>
                    <div style="margin:15px 0; padding:10px; background:#f5f5f5; border-radius:5px; word-break:break-all;">
                      <small>${hpp_url}</small>
                    </div>
                  </div>
                `,
            ...
            primary_action: function () {
              window.open(hpp_url, "_blank");
              d.hide();

              // Start polling for completion
              if (r.message.transaction_log) {
            ...
            secondary_action: function () {
              frappe.utils.copy_to_clipboard(hpp_url);
          ...
          mobile_payments._show_error_dialog(
            (r.message && r.message.message) || "Failed to create payment page"
          );
        }
      },
    });
