 ...
 * Adds "Pay with Mobile" button to Sales Invoice, Payment Entry, Patient Appointment, and POS.
...
// ──────────────────────────────────────────────
// Payment Entry Integration – "Mobile Money" Mode of Payment
// ──────────────────────────────────────────────

frappe.ui.form.on("Payment Entry", {
  refresh: function (frm) {
    mobile_payments._toggle_pe_mobile_button(frm);

    // Show mobile payment indicator if transaction is linked
    if (frm.doc.mobile_payment_transaction_id) {
      frm.dashboard.set_headline(
        `<span class="indicator whitespace-nowrap green">
          <i class="fa fa-mobile"></i> ${__("Mobile Payment")}: 
          <a href="/app/mobile-payment-transaction-log/${frm.doc.mobile_payment_transaction_id}">
            ${frm.doc.mobile_payment_transaction_id}
          </a>
          ${frm.doc.mobile_payment_reference ? " — Ref: " + frm.doc.mobile_payment_reference : ""}
        </span>`
      );
    }
  },
...
var mobile_payments = {
    ...
    let d = new frappe.ui.Dialog({
      ...
      fields: [
          ...
          options: dialog_css + `
            <div class="mp-amount-header">
              ...
              <div class="mp-inner">
                ...
                <p class="mp-invoice"><i class="fa fa-file-text-o" style="margin-right:4px;"></i>${opts.doctype === "Payment Entry" ? __("Payment Entry") : __("Invoice")} #${invoice_label}</p>
              </div>
            </div>
        ...
        },
        {
          ...
          hidden: opts.doctype === "Payment Entry" ? 1 : 0,
      ...
      primary_action: function (values) {
        ...
        if (!is_hpp) {
          ...
          } else if (opts.doctype === "Payment Entry") {
            mobile_payments._initiate_pe_payment(frm, provider, method, phone, currency);
          } else {
            mobile_payments._initiate_purchase_payment(frm, provider, method, phone, currency);
          }
        } else {
          d.hide();
          if (opts.doctype === "Payment Entry") {
            // HPP / Payment Links are managed from the Sales Invoice, not the PE.
            frappe.msgprint({
              title: __("Not Available"),
              message: __("Payment Links can only be generated from the Sales Invoice. "
                + "Please use <strong>USSD Push</strong> to collect payment directly from this Payment Entry."),
              indicator: "orange",
            });
          } else if (opts.doctype === "Patient Appointment") {
  ...
  /**
   * Toggle the "Pay via Mobile" button on Payment Entry forms.
   * Shown only when: draft, Mode of Payment matches mobile money, paid_amount > 0.
   */
  _toggle_pe_mobile_button: function (frm) {
    // Remove any previous instance so we don't stack duplicates
    frm.remove_custom_button(__("Pay via Mobile"), __("Payment"));

    if (frm.doc.docstatus !== 0) return;
    if (frm.is_new()) return;
    if (flt(frm.doc.paid_amount) <= 0) return;

    // Skip if a mobile payment was already linked to this PE
    if (frm.doc.mobile_payment_transaction_id) return;

    // Accept common mobile-money Mode of Payment names
    let mop = (frm.doc.mode_of_payment || "").toLowerCase().replace(/[\s_-]/g, "");
    let mobile_modes = [
      "mobilemoney", "mobile", "zaad", "evcplus", "evc", "edahab",
      "sahal", "waafipay", "waafi",
    ];
    let is_mobile = mobile_modes.some(function (m) { return mop.indexOf(m) !== -1; });
    if (!is_mobile) return;

    frm.add_custom_button(
      __("Pay via Mobile"),
      function () {
        mobile_payments.show_payment_entry_dialog(frm);
      },
      __("Payment")
    );

    // Promote the button colour so it's visually prominent
    frm.change_custom_button_type(__("Pay via Mobile"), __("Payment"), "primary");
  },

  /**
   * Show the mobile payment dialog for a draft Payment Entry.
   */
  show_payment_entry_dialog: function (frm) {
    frappe.call({
      method: "mobile_payments.utils.payment_handler.get_available_methods",
      callback: function (r) {
        if (!r.message || !r.message.enabled) {
        ...
        // Try to fetch customer phone
        let customer = frm.doc.party_type === "Customer" ? frm.doc.party : "";
        let default_provider = (methods[0] || {}).provider || "";

        if (customer) {
          frappe.call({
            method: "mobile_payments.api.pos.get_customer_phone_for_provider",
            args: { customer: customer, provider: default_provider },
            callback: function (phone_r) {
              let phone_data = phone_r.message || {};
              mobile_payments._show_provider_selection(frm, methods, {
                amount: flt(frm.doc.paid_amount),
  ...
  /**
   * Initiate USSD Push payment for a Payment Entry (draft).
   */
  _initiate_pe_payment: function (frm, provider, method, phone, currency) {
    let processing_dialog = mobile_payments._show_processing_dialog(
      provider, method,
    ...
    frappe.call({
      method: "mobile_payments.utils.payment_handler.initiate_payment_entry_mobile_payment",
      args: {
