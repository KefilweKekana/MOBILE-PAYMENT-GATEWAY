...
// ──────────────────────────────────────────────
// Intercept "Enter Payment Details" dialog on Patient Appointment
// When user selects a mobile payment mode (ZAAD, Edahab, etc.),
// automatically open our mobile payment dialog instead.
// ──────────────────────────────────────────────

$(document).on('shown.bs.modal', function () {
  if (!cur_frm || cur_frm.doctype !== 'Patient Appointment') return;
  if (!cur_dialog || !cur_dialog.fields_dict) return;

  // Only intercept dialogs that have a Mode of Payment field
  let mop_field = cur_dialog.fields_dict.mode_of_payment;
  if (!mop_field || mop_field._mp_hooked) return;
  mop_field._mp_hooked = true;

  // Mode of Payment names that map to mobile payment providers
  let mobile_modes = ['zaad', 'evcplus', 'evc plus', 'edahab', 'sahal', 'waafipay'];

  let on_mode_select = function () {
    let raw = cur_dialog.get_value('mode_of_payment') || '';
    let val = raw.toLowerCase().replace(/[\s_-]/g, '');
    if (!val) return;

    let is_mobile = mobile_modes.some(function (m) {
      return val.indexOf(m.replace(/\s/g, '')) !== -1;
    });

    if (is_mobile) {
      // Close the standard dialog and open our mobile payment dialog
      cur_dialog.hide();
      mobile_payments.show_appointment_payment_dialog(cur_frm);
    }
  };

  // Listen for Link field selection (awesomplete dropdown) and regular change
  mop_field.$input.on('change awesomplete-selectcomplete', on_mode_select);
});
...
var mobile_payments = {
   ...
   */
  /**
   * Get the payable amount in the invoice's OWN (transaction) currency.
   *
   * For a fully-unpaid invoice this returns grand_total (transaction currency).
   * For a partly-paid invoice this returns only the outstanding portion,
   * converted back into the transaction currency when the invoice uses a
   * different currency from the company.
   *
   * ERPNext field reference:
   *   grand_total        — always in TRANSACTION currency
   *   base_grand_total   — always in COMPANY currency
   *   outstanding_amount — always in COMPANY currency
   *   conversion_rate    — transaction → company multiplier
   */
  _get_invoice_amount: function (frm) {
    ...
    var outstanding_company = flt(frm.doc.outstanding_amount || 0);

    // If fully unpaid, return grand_total (avoids rounding drift)
    var base_grand = flt(frm.doc.base_grand_total || 0);
    var is_fully_unpaid = base_grand > 0
      ? Math.abs(outstanding_company - base_grand) < 0.01
      : outstanding_company >= grand;

    if (is_fully_unpaid) {
      // Honour rounded_total for same-currency cosmetic rounding
    ...
    // ── Partly paid: compute outstanding in the TRANSACTION currency ──
    if (invoice_currency === company_currency) {
      // Same currency — outstanding_amount is already in transaction currency
      return outstanding_company;
    }

    // Foreign-currency invoice: scale proportionally
    //   outstanding_txn = grand_total × (outstanding_company / base_grand_total)
    if (base_grand > 0) {
      return flt(grand * (outstanding_company / base_grand), 2);
    }

    // Fallback: use conversion_rate if base_grand_total is missing
    var rate = flt(frm.doc.conversion_rate || 1);
    if (rate > 0) {
      return flt(outstanding_company / rate, 2);
    }
  ...
  show_payment_dialog: function (frm) {
    // Refresh the doc from server first so outstanding_amount is never stale
    frappe.call({
      method: "frappe.client.get_value",
      args: {
        doctype: "Sales Invoice",
        filters: { name: frm.doc.name },
        fieldname: ["outstanding_amount", "grand_total", "base_grand_total",
                     "rounded_total", "conversion_rate", "currency"],
      },
      callback: function (fresh) {
        if (fresh.message) {
          // Patch the live doc with fresh server values
          frm.doc.outstanding_amount = fresh.message.outstanding_amount;
          frm.doc.grand_total = fresh.message.grand_total;
          frm.doc.base_grand_total = fresh.message.base_grand_total;
          frm.doc.rounded_total = fresh.message.rounded_total;
          frm.doc.conversion_rate = fresh.message.conversion_rate;
          frm.doc.currency = fresh.message.currency;
        }
        mobile_payments._show_payment_dialog_inner(frm);
      },
    });
  },

  /** Inner dialog launcher (called after fresh outstanding data is loaded). */
  _show_payment_dialog_inner: function (frm) {
    // Fetch available methods from server
    frappe.call({
  ...
  _show_provider_selection: function (frm, methods, opts) {
    ...
    let dialog_css = `
      <style>
        ...
        .mp-dialog .modal-body { padding: 0 !important; margin: 0 !important; background: #10b981 !important; border: none !important; }
        .mp-dialog .modal-body > .form-layout,
        .mp-dialog .form-layout { padding: 0 !important; margin: 0 !important; border: none !important; background: #fff !important; }
        .mp-dialog .form-page, .mp-dialog .form-layout .form-page { padding: 0 !important; margin: 0 !important; border: none !important; background: #fff !important; }
    ...
    // Determine if partly paid — use correct label
    let is_partly_paid = flt(frm.doc.outstanding_amount || 0) > 0
      && flt(frm.doc.outstanding_amount) < flt(frm.doc.base_grand_total || frm.doc.grand_total || 0);
    let amount_label = is_partly_paid ? __("Outstanding Amount") : __("Total Amount");
    ...
    let d = new frappe.ui.Dialog({
      ...
      fields: [
        {
          ...
          options: dialog_css + `
            <div class="mp-amount-header">
              ...
              <div class="mp-inner">
                <p class="mp-label">${amount_label}</p>
        ...
        },
        {
          ...
          description: '<i class="fa fa-info-circle" style="color:#10b981;"></i> '
            + __("USSD Push: prompt on phone now. Payment Link: share a link for the customer to pay later."),
        },
        {
      ...
      primary_action: function (values) {
        ...
        let is_hpp = values.flow_type.includes("Payment Link");
    ...
    if (d.fields_dict.flow_type && d.fields_dict.flow_type.$input) {
      ...
      $flow.find('option').each(function() {
        ...
        else if (v.indexOf('Payment Link') !== -1) $(this).text(__("Payment Link (Send to Customer)"));
      });
    }
  ...
  _initiate_purchase_payment: function (frm, provider, method, phone, currency) {
    ...
    let args = {
      ...
      description: frm.doc.name ? `Payment for ${frm.doc.name}` : "Mobile Payment",
  ...
  /**
   * Check whether the Payment Entry has a mobile-money Mode of Payment
   * and if so, show the "Pay with Mobile" button (draft only).
   */
  _maybe_show_pe_mobile_button: function (frm) {
    ...
    let mobile_modes = [
      'mobile money', 'zaad', 'evcplus', 'evc plus',
      'edahab', 'sahal', 'waafipay', 'mobile payment',
    ];
    let raw = (frm.doc.mode_of_payment || '').toLowerCase().replace(/[\s_-]/g, '');
    let is_mobile = mobile_modes.some(function (m) {
      return raw.indexOf(m.replace(/\s/g, '')) !== -1;
    });
    if (!is_mobile) return;
      ...
      function () {
        if (si_ref) {
          ...
          frappe.model.with_doc("Sales Invoice", si_ref.reference_name, function () {
            let si_frm = { doc: frappe.get_doc("Sales Invoice", si_ref.reference_name) };
            // Monkey-patch reload_doc so the success dialog reloads the PE
            si_frm.reload_doc = function () { frm.reload_doc(); };
            mobile_payments.show_payment_dialog(si_frm);
          });
        } else {
          // No linked SI — show a standalone dialog to collect phone & provider
          mobile_payments._show_pe_standalone_payment_dialog(frm);
        }
      }
    );

    // Make the button stand out as a primary action (no group = standalone button)
    frm.change_custom_button_type(__("Pay with Mobile"), null, "primary");
  },

  /**
   * Standalone mobile payment dialog for a Payment Entry that has no
   * linked Sales Invoice reference (e.g. an advance payment).
   * Initiates a USSD Push directly.
   */
  _show_pe_standalone_payment_dialog: function (frm) {
    frappe.call({
      method: "mobile_payments.utils.payment_handler.get_available_methods",
      callback: function (r) {
        if (!r.message || !r.message.enabled) {
        ...
        let amount = flt(frm.doc.paid_amount || frm.doc.received_amount || 0);
        let currency = frm.doc.paid_to_account_currency
        ...
        let fetch_phone_promise;
        if (party) {
          fetch_phone_promise = new Promise(function (resolve) {
            frappe.call({
              method: "mobile_payments.api.pos.get_customer_phone_for_provider",
          ...
          fetch_phone_promise = Promise.resolve("");
        }

        fetch_phone_promise.then(function (prefill_phone) {
          let d = new frappe.ui.Dialog({
            ...
            fields: [
              {
                ...
                fieldname: "pe_info",
                options: `
                  <div style="text-align:center; margin-bottom:16px; padding:16px; background:#f0fdf4; border-radius:10px; border:1px solid #bbf7d0;">
            ...
            primary_action: function (values) {
              ...
              let api_method = provider === "Edahab"
                ? "mobile_payments.api.edahab.initiate_edahab_payment"
                : "mobile_payments.api.waafipay.initiate_waafipay_payment";
              ...
              frappe.call({
                ...
                callback: function (cr) {
                  if (cr.message) {
                    let result = cr.message;
                    if (result.success) {
                      processing_dialog.hide();
                      // Link the transaction to this Payment Entry for later reconciliation
                      if (result.transaction_log) {
                        frm.set_value("mobile_payment_transaction_id", result.transaction_log);
                      }
                      mobile_payments._show_success_dialog(result, frm);
                    } else if (result.pending) {
                      mobile_payments._start_status_polling(
                        result.transaction_log, processing_dialog, frm
                      );
                    } else {
                      processing_dialog.hide();
                      mobile_payments._show_error_dialog(result.message || "Payment failed");
                    }
                  }
                },
                error: function () {
                  processing_dialog.hide();
                  mobile_payments._show_error_dialog("An error occurred while processing the payment");
                },
              });
            },
          });
...
// Payment Entry integration is handled by doctype_js hook
// (see public/js/payment_entry.js)
