...
mobile_payments.pos = {
  ...
  _get_currency: function () {
    try {
      ...
      // 3. Vue 3 — check __vue_app__ and __vueParentComponent directly
      var roots = document.querySelectorAll(
        "#pos-awesome-root, .pos-awesome-app, [id^='posa'], [class*='posa-app'], [data-app], .v-application, #app"
      );
      for (var i = 0; i < roots.length; i++) {
      ...
      // 4. Read from the payment panel DOM — look for the currency selector/display
      var currencySelectors = [
        ".posa-currency", "[class*='currency-label']", ".pos-currency",
        "select[class*='currency']", ".v-select__selection",
      ];
      for (var c = 0; c < currencySelectors.length; c++) {
        var els = document.querySelectorAll(currencySelectors[c]);
        for (var e = 0; e < els.length; e++) {
          var ct = (els[e].textContent || els[e].value || "").trim().toUpperCase();
          if (ct && ct.length <= 5 && /^[A-Z]{2,5}$/.test(ct)) return ct;
        }
      }

      // 5. Look for "Currency" label in the POS form and read its value
      var labels = document.querySelectorAll(".v-label, label, .label, .v-select__selection");
      for (var l = 0; l < labels.length; l++) {
        var lt = (labels[l].textContent || "").trim().toLowerCase();
        if (lt === "currency") {
          var parent = labels[l].closest(".v-input, .form-group, .field, .v-select");
          if (parent) {
            var val = parent.querySelector(".v-select__selection, input, .value");
            if (val) {
              var vt = (val.textContent || val.value || "").trim().toUpperCase();
              if (vt && vt.length <= 5 && /^[A-Z]{2,5}$/.test(vt)) return vt;
            }
          }
        }
      }

      // 6. Scan visible text for known currency codes near amounts
      var bodyText = document.body.innerText || "";
      var slshMatch = bodyText.match(/\b(SLSH|SOS)\b/i);
      if (slshMatch) return slshMatch[1].toUpperCase();
