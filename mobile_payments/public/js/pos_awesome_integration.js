...
mobile_payments.pos = {
  ...
  _delegate_submit_print: function () {
    if (!mobile_payments.pos._payment_verified) return;
    // Prevent double-submit (countdown + button both firing, or two clicks)
    if (mobile_payments.pos._submitting) return;
    mobile_payments.pos._submitting = true;

    frappe.show_alert({ message: __("Submitting invoice…"), indicator: "blue" }, 3);

    // Get the full POS invoice doc from Vue store — POS Awesome needs the
    // full doc object (not just name) to submit and generate the invoice name.
    var pos_doc = mobile_payments.pos._get_pos_doc();

    if (!pos_doc) {
      // Can't find the doc — fall back to clicking POS Awesome's button
    ...
    // Step 1: Fetch the FRESH doc from server (avoids stale timestamp mismatch)
    frappe.call({
      method: "frappe.client.get",
      args: { doctype: "POS Invoice", name: invoice_name },
      callback: function (r) {
        if (!r || !r.message) {
          // Try Sales Invoice doctype as fallback
          frappe.call({
            method: "frappe.client.get",
            args: { doctype: "Sales Invoice", name: invoice_name },
            callback: function (r2) {
              ...
              mobile_payments.pos._do_submit(r2.message);
            },
          });
          return;
        }
        mobile_payments.pos._do_submit(r.message);
      },
    });
  },

  // Step 2: Submit the fresh doc and open print
  _do_submit: function (fresh_doc) {
    frappe.call({
      ...
      callback: function (r) {
        ...
        var submitted_name = (r && r.message && r.message.name)
          ? r.message.name : fresh_doc.name;
        ...
        var print_url = "/printview?doctype=Sales%20Invoice"
          + "&name=" + encodeURIComponent(submitted_name)
          + "&trigger_print=1"
          + "&format=undefined"
          + "&no_letterhead=0";
