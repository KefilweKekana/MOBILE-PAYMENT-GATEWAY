...
function shouldCommitPreauth(mpTxn) {
    const kind = (mpTxn?.authorization_type || mpTxn?.txn_type || "").toUpperCase();
    // commit only if this txn is a preauth flow
    return kind === "PREAUTH" || kind === "PREAUTHORIZE";
}

async function finalizePaidPosInvoice({ invoice_name, provider_txn_id, mp_txn }) {
    // 1) Commit only when required
    if (shouldCommitPreauth(mp_txn)) {
        const r = await frappe.call({
            method: "mobile_payments.api.pos.commit_preauthorize",
            args: { invoice_name, provider_txn_id }
        });
        const m = r.message || {};
        if (!(m.ok || m.idempotent || m.skipped)) {
            throw new Error(m.error || "Preauthorize commit failed");
        }
    }

    // 2) Always continue submit + print after successful payment
    await frappe.call({
...
async function submitAndPrintAfterPayment(invoiceDoc) {
    // invoiceDoc should be the POS invoice JSON used by posawesome submit API
    const submitRes = await frappe.call({
        method: "posawesome.posawesome.api.invoices.submit_invoice",
        args: { invoice: JSON.stringify(invoiceDoc) }
    });

    const submitted = submitRes?.message;
    const invoiceName = submitted?.name || invoiceDoc?.name;
    if (!invoiceName) throw new Error("Invoice submitted but name is missing.");

    // Open print view (new tab)
    const printUrl =
        `/printview?doctype=Sales%20Invoice&name=${encodeURIComponent(invoiceName)}&trigger_print=1&format=${encodeURIComponent("POS Invoice")}&no_letterhead=0`;
    window.open(printUrl, "_blank");
}

// Call this INSIDE your payment-success callback
async function onMobilePaymentSuccess(invoiceDoc) {
    try {
        await submitAndPrintAfterPayment(invoiceDoc);
        frappe.show_alert({ message: __("Payment successful. Invoice submitted and sent to print."), indicator: "green" });
    } catch (e) {
        // show exact backend reason (important for 417 valuation errors)
        const msg =
            e?.message ||
            __("Payment succeeded, but Submit/Print failed. Check stock valuation / invoice validation.");
        frappe.msgprint({
            title: __("Submit/Print Failed"),
            indicator: "red",
            message: msg
        });
    }
