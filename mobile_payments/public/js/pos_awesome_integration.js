...
mobile_payments.pos = {
  ...
  _delegate_submit_print: function () {
    ...
    if (!invoice) {
      // No invoice name yet — fall back to clicking POS button
      frappe.show_alert({
    ...
    frappe.call({
      ...
      callback: function (r) {
        ...
        // Success — figure out the final invoice name (may have changed on submit)
        var submitted_name = (r && r.message && r.message.name) ? r.message.name : invoice;
        ...
        // Open print view in new tab exactly like POS Awesome does
        ...
        // Reset POS state for next transaction
        mobile_payments.pos._payment_verified    = false;
        mobile_payments.pos._verified_mop_label  = "";
