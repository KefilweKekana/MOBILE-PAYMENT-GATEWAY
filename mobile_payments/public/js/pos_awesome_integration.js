...
mobile_payments.pos = {
  ...
  _get_pos_outstanding_amount: function () {
    ...
    try {
      ...
      // From POS display
      let grandTotalEl = document.querySelector(
        ".grand-total .value, .pos-grand-total, [data-grand-total]"
      );
      if (grandTotalEl) {
        return flt(grandTotalEl.textContent.replace(/[^0-9.]/g, ""));
      }
    } catch (e) {
  ...
  _get_pos_customer: function () {
    ...
    try {
      ...
      // Try to get phone number from customer
      if (info.name) {
        // Async would be better but we need it now
        frappe.call({
