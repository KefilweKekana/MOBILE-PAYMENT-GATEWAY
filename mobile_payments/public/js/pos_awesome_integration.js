/**
 * Mobile Payments — POS Awesome Integration  (v39 — V14/V15 compatible)
 *
 * Strategy:
 ...
 *        our payment dialog
 *     3. Direct button-click listener — catches mobile MOP button clicks
 *        immediately (V14 fallback where poll may miss DOM updates)
 *     4. Guard SUBMIT — if mobile MOP has amount but not verified, block and
 *        show the dialog
 *     5. On success → _payment_verified = true → SUBMIT goes through
 *
 *   V14 vs V15 DOM differences:
 *     - V15 uses Vuetify `.v-row` containers for payment mode rows
 *     - V14 uses `.row`, `.payment-mode-row`, or other wrappers
 *     - _get_payment_rows() tries multiple selectors + button-walk fallback
 */
...
mobile_payments.pos = {
  ...
  _payment_panel_open: function () {
    ...
    // Fallback: look for payment-related buttons that are visible
    var payRe = /^(cash|zaad|sahal|evcplus|edahab|waafipay|rec\s*cash|submit)/i;
  ...
  _get_payment_rows: function () {
    ...
    // Strategy: find payment-mode button+input pairs inside row-like containers.
    // V15 POS Awesome uses Vuetify `.v-row`, V14 uses various wrappers
    // (`.row`, `.payment-mode-row`, `.v-list-item`, `.col-*`, div wrappers).
    // We try multiple selectors, then fall back to a button-walk approach.
    var containerSelectors = [
      ".v-row",               // Vuetify 2/3 (V15)
      ".payment-mode-row",    // some POS Awesome versions
      ".v-list-item",         // Vuetify list layout
      ".row",                 // Bootstrap row
    ];
    ...
    // Fallback: walk from each visible POS-area button upward to find a
    // sibling or nearby input. Covers V14 layouts that don't use standard
    // row containers.
    if (rows.length === 0) {
      var allBtns = document.querySelectorAll("button, .v-btn");
      allBtns.forEach(function (btn) {
        ...
        var label = (btn.textContent || "").trim().toUpperCase();
        if (!label) return;
        // Skip generic buttons (close, save, menu, etc.)
        if (/^(SAVE|CLOSE|CANCEL|MENU|OK|YES|NO|PRINT)$/.test(label)) return;
        // Walk up to 4 parent levels to find a sibling input
        var node = btn;
        var inp = null;
        for (var depth = 0; depth < 4 && node && node !== document.body; depth++) {
          node = node.parentElement;
          if (!node) break;
          inp = node.querySelector("input");
          if (inp && inp !== btn) break;
          inp = null;
        }
        if (!inp) return;
        seen.add(btn);
        rows.push({ btn: btn, inp: inp, label: label, row: node });
      });
    }

    // Fallback for Payments tab: look for label+input pairs where the label
    // is plain text (not a button) next to an input field, inside any container.
    // The Payments tab often uses <div>...<label>ZAAD:</label><input>...</div> or
    // similar structures without button elements.
    if (rows.length === 0) {
      var seenInputs = new Set();
      var allInputs = document.querySelectorAll("input[type='text'], input[type='number'], input:not([type])");
      allInputs.forEach(function (inp) {
        if (seenInputs.has(inp)) return;
        var rect = inp.getBoundingClientRect();
        if (rect.width === 0 || rect.height === 0) return;

        // Walk up to find parent container, then look for a label-like element
        var container = inp.parentElement;
        for (var depth = 0; depth < 5 && container && container !== document.body; depth++) {
          // Look for text content in this container that matches a mobile MOP
          var textEls = container.querySelectorAll("label, span, div, p, .v-label, .v-list-item__title");
          for (var t = 0; t < textEls.length; t++) {
            var txt = (textEls[t].textContent || "").replace(/[:\s]/g, "").toUpperCase();
            if (!txt) continue;
            if (mobile_payments.pos._is_mobile_mop(txt) || /^CASH$/i.test(txt)) {
              seenInputs.add(inp);
              rows.push({ btn: textEls[t], inp: inp, label: txt, row: container });
              return; // found match for this input, move on
            }
          }
  ...
  init: function () {
    ...
    console.log("mobile_payments.pos.init() — v39 poll+click (V14/V15)");
    ...
    mobile_payments.pos._attach_button_click_listener();
  },

  // ── A0. Direct button-click listener (V14 compatibility) ──────
  //
  // On V14, clicking ZAAD/EDAHAB buttons may not trigger the poll-based
  // watcher reliably (different DOM update timing). So we also listen for
  // direct clicks on mobile-MOP buttons and auto-open the dialog after
  // a short delay to let POS Awesome set the amount.
  //
  _button_click_attached: false,
  _attach_button_click_listener: function () {
    if (mobile_payments.pos._button_click_attached) return;
    mobile_payments.pos._button_click_attached = true;
    ...
    document.addEventListener("click", function (e) {
      ...
      if (mobile_payments.pos._just_submitted) return;
      ...
      // This is a click on a mobile MOP button (ZAAD, EDAHAB, etc.)
      // Wait briefly for POS Awesome to set the amount, then open dialog
      console.log("mobile_payments.pos: mobile MOP button clicked:", label);
      setTimeout(function () {
        if (mobile_payments.pos._dialog_open) return;
        if (mobile_payments.pos._payment_verified) return;

        // Find the amount — check the payment rows
        var amount = 0;
        var rows = mobile_payments.pos._get_payment_rows();
        for (var i = 0; i < rows.length; i++) {
          if (rows[i].label === label || rows[i].label.indexOf(label) !== -1 || label.indexOf(rows[i].label) !== -1) {
            amount = mobile_payments.pos._parse_amount(rows[i].inp.value);
            break;
          }
        }

        // If no amount detected from the row, try the outstanding amount
        if (amount <= 0) {
          amount = mobile_payments.pos._get_outstanding();
        }

        if (amount > 0) {
          console.log("mobile_payments.pos: button-click trigger:", label, "amount:", amount);
          mobile_payments.pos._prev_mobile_amount = amount;
          mobile_payments.pos._prev_mobile_label = label;
          ...
          mobile_payments.pos.show_dialog(methodObj, amount, btn);
        }
      }, 300);
    }, false);
  },
  ...
  _full_reset: function () {
    ...
    mobile_payments.pos._update_submit_buttons();
    // Final cleanup: remove any stale backdrops left by rapid dialog transitions
    $(".modal-backdrop.mobile-payment-backdrop").remove();
  ...
  show_dialog: function (methodObj, amount, clickedBtn) {
    ...
    var d = new frappe.ui.Dialog({
      ...
      primary_action: function (values) {
        ...
        mobile_payments.pos._dialog_open = false;
        mobile_payments.pos._force_close_dialog(d);
        ...
        var method     = parts[1];
        mobile_payments.pos._process(provider, method, phone_clean, values.amount, currency, invoice, customer, "Subscriber (Mobile Wallet)");
      },
    });
    ...
    d.show();
    // Ensure dialog appears above POS Awesome payment overlay
    d.$wrapper.addClass("mobile-payment-modal");
    $(".modal-backdrop").last().addClass("mobile-payment-backdrop");
  ...
  _process: function (provider, method, phone, amount, currency, invoice, customer, account_type) {
    ...
    spin.show();
    spin.$wrapper.addClass("mobile-payment-modal");
    $(".modal-backdrop").last().addClass("mobile-payment-backdrop");
    ...
    frappe.call({
      ...
      callback: function (r) {
        ...
        if (res.success) {
          mobile_payments.pos._force_close_dialog(spin);
  ...
  _poll: function (txlog, dialog, amount, method) {
    ...
    dialog.set_primary_action(__("Cancel"), function () {
      clearInterval(timer);
      mobile_payments.pos._force_close_dialog(dialog);
  ...
  _finish_submit: function (submitted_name, pos_invoice_name) {
    ...
    // Dismiss POS Awesome's payment overlay by clicking "CANCEL PAYMENT"
    // This triggers POS Awesome's own cleanup (new invoice, clear cart, etc.)
    mobile_payments.pos._close_pos_payment_overlay();
  ...
  /**
   * Dismiss POS Awesome's payment overlay by clicking its own buttons.
   * Tries "CANCEL PAYMENT" first, then auto-confirms the "Cancel Sale?"
   * dialog that POS Awesome shows. Since the invoice is already submitted,
   * this just clears POS Awesome's internal state for the next transaction.
   */
  _close_pos_payment_overlay: function () {
    var clicked = false;
    document.querySelectorAll("button, .v-btn").forEach(function (btn) {
      if (clicked) return;
      var rect = btn.getBoundingClientRect();
      if (rect.width === 0 || rect.height === 0) return;
      var label = (btn.textContent || "").replace(/\s+/g, " ").trim().toUpperCase();
      if (label === "CANCEL PAYMENT" || label === "CANCEL SALE") {
        btn.click();
        clicked = true;
        console.log("mobile_payments: clicked", label, "to dismiss POS overlay");
      }
    });

    if (clicked) {
      // POS Awesome shows a "Cancel Sale?" confirmation — auto-click YES
      var _confirm_attempts = 0;
      var _confirm_timer = setInterval(function () {
        _confirm_attempts++;
        if (_confirm_attempts > 10) { clearInterval(_confirm_timer); return; }

        document.querySelectorAll("button, .v-btn, .v-card__actions button").forEach(function (btn) {
          var label = (btn.textContent || "").replace(/\s+/g, " ").trim().toUpperCase();
          if (label === "YES, CANCEL SALE" || label === "YES,CANCEL SALE"
              || label === "YES" || label === "OK") {
            var rect = btn.getBoundingClientRect();
            if (rect.width > 0 && rect.height > 0) {
              btn.click();
              clearInterval(_confirm_timer);
              console.log("mobile_payments: auto-confirmed Cancel Sale dialog");
            }
          }
        });
      }, 200);
    } else {
      // Fallback: try to close via Escape key on the payment panel
      try {
        document.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", keyCode: 27, bubbles: true }));
        console.log("mobile_payments: dispatched Escape to dismiss POS overlay");
      } catch (e) {}
    }
  },
  ...
  /**
   * Force-close a Frappe dialog and clean up orphaned Bootstrap backdrops.
   * Bootstrap's async modal-hide gets confused when dialogs are shown/hidden
   * rapidly (payment → spinner → success) at high z-index, leaving stale
   * .modal-backdrop elements that grey-out the page.
   */
  _force_close_dialog: function (dlg) {
    if (!dlg || !dlg.$wrapper) return;
    try {
      dlg.$wrapper.removeClass("show in fade").css("display", "none");
      dlg.$wrapper.remove();
    } catch (e) {}
    // Remove any orphaned backdrops we created
    $(".modal-backdrop.mobile-payment-backdrop").remove();
    // If no other modals are visible, clean up body modal state
    if (!$(".modal.show").length && !$(".modal.in").length) {
      $("body").removeClass("modal-open").css("padding-right", "");
    }
  },
  ...
  _on_error: function (message) {
    ...
    mobile_payments.pos._verified_mop_label = "";
    mobile_payments.pos._cooldown_until = Date.now() + 2000;
    // Clean up any stale backdrops before showing error
    $(".modal-backdrop.mobile-payment-backdrop").remove();
    if (!$(".modal.show").length && !$(".modal.in").length) {
      $("body").removeClass("modal-open").css("padding-right", "");
    }
    ...
    // Use frappe.ui.Dialog instead of frappe.msgprint so we can apply
    // the z-index class immediately — msgprint shows behind POS Awesome
    var errDlg = new frappe.ui.Dialog({
      ...
      fields: [{
        ...
        options:
          ...
          + '<div style="width:64px; height:64px; border-radius:50%; background:#f8d7da;'
          + ' margin:0 auto 14px; display:flex; align-items:center; justify-content:center;">'
          + '<i class="fa fa-times" style="font-size:30px; color:#dc3545;"></i></div>'
          + '<p style="font-size:15px; color:#333; margin-bottom:6px;">' + message + '</p>'
          + '<p class="text-muted" style="font-size:13px;">' + __("Please try again.") + '</p>'
          + '</div>',
      }],
      primary_action_label: __("OK"),
      primary_action: function () { errDlg.hide(); },
    });
    errDlg.show();
    errDlg.$wrapper.addClass("mobile-payment-modal");
    $(".modal-backdrop").last().addClass("mobile-payment-backdrop");
  },
