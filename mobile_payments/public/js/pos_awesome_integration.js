...
mobile_payments.pos = {
  ...
  show_dialog: function (methodObj, amount, clickedBtn) {
    ...
    // If currency is not USD, convert the amount using the POS conversion rate
    // POS Awesome stores amounts internally in USD — we need the actual local currency amount
    var display_amount = amount;
    if (currency && currency.toUpperCase() !== "USD") {
      var rate = mobile_payments.pos._get_conversion_rate();
      if (rate > 1) {
        display_amount = amount * rate;
        console.log("mobile_payments.pos: converted", amount, "USD ×", rate, "=", display_amount, currency);
      }
    }
