...
mobile_payments.pos = {
  ...
  _try_inject_now: function () {
    ...
    // Vuetify POS: look for payment buttons (ZAAD, EDAHAB, etc.) or .pay-btn
    let hasPaymentBtns = document.querySelector(".pay-btn, .summary-btn");
    if (!hasPaymentBtns) {
      // Also check for native payment method buttons
      let allBtns = document.querySelectorAll("button, .v-btn");
      for (let b of allBtns) {
        let t = (b.textContent || "").trim().toUpperCase();
        if (["REC CASH","ZAAD","SAHAL","WAAFIPAY","EDAHAB","EVCPLUS"].includes(t)) {
          hasPaymentBtns = b; break;
        }
      }
    }
    if (hasPaymentBtns) {
   ...
   * Vuetify-based POS System — with an amount input on the left and a
   * styled button on the right, just like REC CASH / ZAAD / EDAHAB.
   */
  _add_mobile_money_button_vuetify: function () {
    ...
    // ── 1. Find the right-side button container ──
    // Locate buttons labeled REC CASH, ZAAD, WAAFIPAY, EDAHAB to find their container
    ...
    // ── 2. Find the left-side input container ──
    // Locate input fields labeled Edahab, ZAAD, WaafiPay, etc.
    let allInputWrappers = document.querySelectorAll(
      ".v-input, .v-text-field, .v-field, [class*='input'], [class*='field']"
    );
    let lastNativeInput = null;
    let inputContainer = null;

    // Search by label text near inputs
    let allLabels = document.querySelectorAll("label, .v-label, .v-field-label, span, div");
    for (let lbl of allLabels) {
      let text = (lbl.textContent || "").trim();
      if (/^(Edahab|WaafiPay|ZAAD|SAHAL|EVCPlus|REC CASH)$/i.test(text)) {
        // Walk up to find the input wrapper and its container
        let wrapper = lbl.closest(".v-input, .v-text-field, [class*='col'], [class*='field']");
        if (wrapper) {
          lastNativeInput = wrapper;
          inputContainer = wrapper.parentElement;
        }
      }
    }

    // ── 3. Create the MOBILE PAYMENT button (right side) ──
    if (btnContainer && lastNativeBtn) {
      let mobileBtn = document.createElement("button");
      mobileBtn.type = "button";
      mobileBtn.className = "mobile-payment-pos-btn";

      // Copy computed style from one of the native buttons
      let refStyle = window.getComputedStyle(lastNativeBtn);
      mobileBtn.style.cssText = `
        ...
        width: ${refStyle.width || "100%"};
        min-height: ${refStyle.minHeight || refStyle.height || "44px"};
        height: ${refStyle.height || "auto"};
        border-radius: ${refStyle.borderRadius || "4px"};
        margin-top: ${refStyle.marginTop || "0"};
        margin-bottom: ${refStyle.marginBottom || "8px"};
        padding: ${refStyle.padding || "8px 16px"};
        text-transform: uppercase;
        letter-spacing: 0.5px;
      `;
      // Also copy native button's class list for Vuetify sizing
      if (lastNativeBtn.className) {
        let nativeClasses = lastNativeBtn.className.split(" ").filter(function(c) {
          return c.startsWith("v-btn") || c.startsWith("v-theme");
        });
        if (nativeClasses.length) {
          mobileBtn.className += " " + nativeClasses.join(" ");
        }
      }
      mobileBtn.innerHTML = '<span class="v-btn__content">MOBILE PAYMENT</span>';
      ...
      // Click → open payment dialog with the amount from the input field
      mobileBtn.addEventListener("click", function (e) {
        e.preventDefault();
        e.stopPropagation();
        // Read amount from our custom input
      ...
      // Insert after the last native button
      if (lastNativeBtn.nextSibling) {
        btnContainer.insertBefore(mobileBtn, lastNativeBtn.nextSibling);
      } else {
        btnContainer.appendChild(mobileBtn);
      }
    }

    // ── 4. Create the MOBILE PAYMENT amount input (left side) ──
    if (inputContainer && lastNativeInput) {
      // Clone the structure from a native input for perfect matching
      let inputWrapper = lastNativeInput.cloneNode(true);
      inputWrapper.className = lastNativeInput.className + " mobile-payment-input-wrapper";
      inputWrapper.id = "mobile-payment-input-row";

      // Find the label inside and change it
      let labels = inputWrapper.querySelectorAll("label, .v-label, .v-field-label");
      for (let lbl of labels) {
        lbl.textContent = "Mobile Payment";
        lbl.style.color = "#2ecc71";
        lbl.style.fontWeight = "600";
      }

      // Find the actual input and reset it
      let input = inputWrapper.querySelector("input");
      if (input) {
        input.id = "mobile-payment-amount-input";
        input.value = "0.00";
        input.name = "mobile_payment_amount";
        // Remove any Vue bindings / readonly
        input.removeAttribute("readonly");
        input.removeAttribute("disabled");
        // Re-attach clean event listeners
      ...
      // Insert after the last native input
      if (lastNativeInput.nextSibling) {
        inputContainer.insertBefore(inputWrapper, lastNativeInput.nextSibling);
      } else {
        inputContainer.appendChild(inputWrapper);
      }
    } else if (!inputContainer) {
      // Fallback: couldn't find input container, create a standalone input
      // near the button
      mobile_payments.pos._create_fallback_input();
    }

    // ── 5. If button container was not found, do a broad fallback ──
    if (!btnContainer) {
      mobile_payments.pos._create_fallback_button();
    }
  },

  /**
   * Fallback: create a standalone amount input if native input container wasn't found.
   */
  _create_fallback_input: function () {
    let mobileBtn = document.querySelector(".mobile-payment-pos-btn");
    if (!mobileBtn) return;

    let inputDiv = document.createElement("div");
    inputDiv.id = "mobile-payment-input-row";
    inputDiv.style.cssText = "margin: 8px 0; padding: 0 4px;";
    inputDiv.innerHTML = `
      <label style="display:block; font-size:12px; font-weight:600; color:#2ecc71; margin-bottom:2px;">
        Mobile Payment
      </label>
      <div style="display:flex; align-items:center; border:1px solid #ccc; border-radius:4px; padding:6px 10px; background:#fff;">
        <span style="color:#666; margin-right:4px;">$</span>
        <input id="mobile-payment-amount-input" type="number" step="0.01" min="0" value="0.00"
          style="border:none; outline:none; width:100%; font-size:16px; font-weight:500; background:transparent;" />
      </div>
    `;

    // Try to place it near the left column where other inputs are
    let leftCol = document.querySelector(".v-col:first-child, [class*='col']:first-child");
    if (leftCol) {
      leftCol.appendChild(inputDiv);
    } else {
      // Place above the button
      mobileBtn.parentElement.insertBefore(inputDiv, mobileBtn);
    }
  },

  /**
   * Fallback: create a standalone button if native button container wasn't found.
   */
  _create_fallback_button: function () {
    if (document.querySelector(".mobile-payment-pos-btn")) return;

    let methods = mobile_payments.pos._methods;
    if (!methods || !methods.length) return;

    // Look for any sensible container: SUBMIT area, pay-btn area, etc.
    let container = document.querySelector(
      ".pay-btn, .summary-btn, [class*='submit'], [class*='payment']"
    );
    if (!container) container = document.querySelector(".v-card, .v-sheet");
    if (!container) return;
    ...
    mobileBtn.style.cssText = `
      ...
      border: none; width: 100%; min-height: 44px; border-radius: 4px;
    ...
    `;
    mobileBtn.innerHTML = '<span>MOBILE PAYMENT</span>';
    ...
    parentEl.insertBefore(mobileBtn, container);

    // Also create the fallback input
    mobile_payments.pos._create_fallback_input();
  },
  ...
  show_pos_payment_dialog: function (preselected_method, amount) {
    ...
    // Try to get amount from our dedicated input field first, then POS
    if (!amount) {
      let amountInput = document.querySelector("#mobile-payment-amount-input");
      if (amountInput) {
