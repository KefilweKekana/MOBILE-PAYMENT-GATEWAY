...
mobile_payments.pos = {
  ...
  /** Programmatically click the SUBMIT button after verified payment. */
  _auto_submit: function () {
    if (!mobile_payments.pos._payment_verified) return;

    // Find the SUBMIT button (not SUBMIT & PRINT)
    var submitBtn = null;
    ...
    document.querySelectorAll("button, .v-btn").forEach(function (btn) {
      ...
      if (label === "SUBMIT & PRINT") submitPrintBtn = btn;
    });

    var target = submitBtn || submitPrintBtn;
    if (target) {
      console.log("mobile_payments.pos: auto-clicking SUBMIT");
      target.click();
    } else {
