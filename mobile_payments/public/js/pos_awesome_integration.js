  ...
  _get_customer_phone_from_vue: function () {
    try {
      ...
      for (var i = 0; i < roots.length; i++) {
        ...
        // Check store state
        var stores = [vue.$store && vue.$store.state];
        if (vue.$children) {
          for (var j = 0; j < vue.$children.length; j++) {
            if (vue.$children[j].$store) stores.push(vue.$children[j].$store.state);
          }
        }
        for (var s = 0; s < stores.length; s++) {
          var st = stores[s];
          if (!st) continue;
          // customer_info or customer object may have phone
          var ci = st.customer_info || st.customer || {};
          if (ci.mobile_no) return ci.mobile_no;
          if (ci.phone) return ci.phone;
          if (ci.contact_mobile) return ci.contact_mobile;
          // invoice doc may have contact_mobile
          if (st.invoice_doc) {
            if (st.invoice_doc.contact_mobile) return st.invoice_doc.contact_mobile;
            if (st.invoice_doc.contact_phone) return st.invoice_doc.contact_phone;
          }
        }
      }
    } catch (e) {
