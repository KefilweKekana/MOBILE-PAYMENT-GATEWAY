    ...
    frappe.call({
      ...
      callback: function (r) {
        ...
        if (mopLabel && fresh.payments && fresh.payments.length) {
          var mopFixed = false;
          // First: if the correct MOP already exists, make sure it has the amount
          ...
          // If the correct MOP isn't in payments, update the Cash entry that has the amount
          if (!mopFixed) {
            for (var p2 = 0; p2 < fresh.payments.length; p2++) {
              var pay = fresh.payments[p2];
              if (pay.amount > 0 && (pay.mode_of_payment || "").toUpperCase() === "CASH") {
                // Find the actual Mode of Payment name (case-sensitive match)
                var correctMop = mobile_payments.pos._find_mop_name(mopLabel);
                if (correctMop) {
                  console.log("mobile_payments.pos: fixing payment mode:", pay.mode_of_payment, "→", correctMop);
                  pay.mode_of_payment = correctMop;
                }
                break;
              }
            }
