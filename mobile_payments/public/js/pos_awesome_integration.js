...
mobile_payments.pos = {
  ...
  _add_mobile_money_button_vuetify: function () {
    ...
    if (!recCashBtn) {
      // Could not find any native payment button, use broad fallback
    ...
    if (clonedBtn) {
      ...
      if (contentSpan) {
        contentSpan.textContent = "MOBILE PAYMENT";
      } else {
        clonedBtn.textContent = "MOBILE PAYMENT";
      }
      ...
      freshBtn.addEventListener("click", function (e) {
        e.preventDefault();
        e.stopPropagation();
        // Read the amount the cashier typed into the Mobile Payment input
        ...
        if (!amount || amount <= 0) {
          frappe.show_alert({
            message: __("Please enter the amount to pay via Mobile Payment"),
            indicator: "orange",
          });
    ...
    row.innerHTML = `
      ...
      <div class="v-col v-col-6" style="padding: 4px;">
        <button type="button" class="mobile-payment-pos-btn v-btn v-btn--block v-btn--elevated v-theme--dark v-btn--density-default v-btn--size-default v-btn--variant-elevated"
          ...
          <span class="v-btn__content">MOBILE PAYMENT</span>
        </button>
      </div>
    `;
    ...
    let d = new frappe.ui.Dialog({
      ...
      fields: [
        ...
        },
        {
          ...
          reqd: 1,
          default: "USD",
          description: __("US Dollar (USD) or Somali Shilling (SLSH)"),
        },
        {
          fieldname: "amount",
          fieldtype: "Currency",
        ...
        if (!info.phone) {
          frappe.call({
            ...
            args: {
              ...
              fieldname: ["mobile_no", "custom_mobile_number"],
            },
            async: false,
            callback: function (r) {
              if (r.message) {
                info.phone = r.message.mobile_no || r.message.custom_mobile_number || "";
              }
            },
          });
        }
