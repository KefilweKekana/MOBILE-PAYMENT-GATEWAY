...
mobile_payments.pos = {
    ...
    frappe.call({
      ...
      callback: function (r) {
        ...
        if (res.success) {
          ...
          // For Edahab HPP — open the payment portal in a new tab
          if (res.hpp_url) {
            spin.hide();
            window.open(res.hpp_url, "_blank");
            // Show a dialog telling cashier to wait for customer to pay
            var d = new frappe.ui.Dialog({
              title: __("Edahab Payment"),
              fields: [{ fieldtype: "HTML", fieldname: "html",
                options: '<div style="text-align:center;padding:20px;">'
                  + '<i class="fa fa-mobile fa-3x" style="color:#28a745;"></i>'
                  + '<p style="margin-top:14px;font-weight:500;">'
                  + __("Payment link opened — waiting for customer to complete payment.")
                  + "</p>"
                  + '<p class="text-muted">' + __("This window will update automatically once paid.") + "</p>"
                  + "</div>",
              }],
            ...
            d.show();
            mobile_payments.pos._poll(res.transaction_log, d, amount, method);
          } else {
  ...
  /**
   * Delegates a click to the real POS Awesome "SUBMIT & PRINT" (or "SUBMIT") button.
   * Because the cashier initiates this click (via our success dialog button),
   * POS Awesome receives it as a genuine user interaction — no preauth conflict.
   */
  _delegate_submit_print: function () {
    ...
    if (target) {
      ...
      // Fallback: POS button not found yet — show a gentle nudge
      frappe.show_alert({
        message: __("Payment verified ✓ — please click SUBMIT & PRINT in the POS panel."),
        indicator: "green",
      }, 8);
    }
  },
