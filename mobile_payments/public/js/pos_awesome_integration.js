...
mobile_payments.pos = {
  ...
  /** Debug helper — logs only when console is open / debug flag is set. */
  _log: function () {
    var args = Array.prototype.slice.call(arguments);
    args.unshift("[MobilePayments POS]");
  ...
  init: function () {
    ...
    mobile_payments.pos._log("init() called on", window.location.href);
    ...
    frappe.call({
      ...
      callback: function (r) {
        if (r.message) {
          ...
          mobile_payments.pos._log("methods loaded:", r.message);
        } else {
          mobile_payments.pos._log("get_mobile_payment_methods returned empty");
        }
      },
      error: function (err) {
        mobile_payments.pos._log("get_mobile_payment_methods error:", err);
      },
    });

    if (!mobile_payments.pos._enabled || !mobile_payments.pos._methods.length) {
      mobile_payments.pos._log("Skipping — enabled:", mobile_payments.pos._enabled,
        "methods:", mobile_payments.pos._methods);
      return;
    }
    ...
    // Start polling for the payment panel (reliable fallback)
    mobile_payments.pos._start_injection_polling();
  },
    ...
    let observer = new MutationObserver(function (mutations) {
      for (let mutation of mutations) {
        for (let node of mutation.addedNodes) {
          ...
          // Look for any v-btn + input row (payment mode pattern)
          let hasPaymentBtn = false;
          if (node.querySelectorAll) {
            let btns = node.querySelectorAll("button, .v-btn");
            for (let b of btns) {
              let t = (b.textContent || "").trim().toUpperCase();
              if (t && (t === "CASH" || t === "REC CASH" || t.indexOf("CASH") !== -1 ||
                  b.closest(".v-row") && b.closest(".v-row").querySelector("input"))) {
                hasPaymentBtn = true; break;
              }
            }
          }
          if (hasPaymentBtn) {
  ...
  _try_inject_now: function () {
    ...
    // Use the structural detection
    let btn = mobile_payments.pos._find_payment_mode_button();
    if (btn) {
      mobile_payments.pos._add_mobile_money_button_vuetify();
      return;
    }
  ...
  /**
   * Start a polling loop that periodically checks whether the payment
   * panel has appeared and injects the MODE OF PAYMENT button if needed.
   * This is the most reliable injection strategy because it doesn't
   * depend on MutationObserver timing or specific event hooks.
   */
  _start_injection_polling: function () {
    if (mobile_payments.pos._polling_started) return;
    mobile_payments.pos._polling_started = true;
    mobile_payments.pos._log("Starting injection polling");

    setInterval(function () {
      if (!mobile_payments.pos._is_pos_page()) return;
      // Only inject when no button exists yet
      if (document.querySelector(".mobile-payment-pos-btn")) return;
      mobile_payments.pos._add_mobile_money_button_vuetify();
    }, 1500);
  },
  ...
  /**
   * Find any existing POS payment-mode button+input row.
   * Uses structural detection: look for visible v-btn elements that sit
   * next to a numeric input inside a row — that pattern is the payment
   * method row in POS Awesome (Vuetify).
   *
   * Returns the button element or null.
   */
  _find_payment_mode_button: function () {
    // Strategy 1 — match by well-known label text (broad list)
    let knownLabels = [
      "CASH", "REC CASH", "BANK", "CARD", "CHEQUE", "CHECK",
      "WIRE TRANSFER", "MPESA", "MOBILE",
      "ZAAD", "SAHAL", "EVCPLUS", "WAAFIPAY", "EDAHAB",
    ];
    let allButtons = document.querySelectorAll("button, .v-btn");
    for (let btn of allButtons) {
      let text = (btn.textContent || "").trim().toUpperCase();
      if (!text) continue;
      for (let label of knownLabels) {
    ...
    // Strategy 2 — structural: any visible .v-btn inside a .v-row that
    // also contains a numeric/text <input>.  Only consider rows whose
    // button text looks like a payment label (all-caps, short, no spaces
    // beyond one or contains a currency-like pattern).
    let rows = document.querySelectorAll(".v-row");
    for (let row of rows) {
      let btn = row.querySelector(".v-btn");
      let inp = row.querySelector("input[type=text], input[type=number], input:not([type])");
      if (btn && inp) {
        let btnText = (btn.textContent || "").trim();
        // Skip rows where the button has no text or very long text (nav buttons)
        if (!btnText || btnText.length > 25) continue;
        // Skip search inputs, item rows, etc.
        if (inp.placeholder && /search|item|qty|quantity|barcode/i.test(inp.placeholder)) continue;
        let r = btn.getBoundingClientRect();
        if (r.width > 0 && r.height > 0) return btn;
      }
    }
   ...
   * the first payment-mode row so it appears right next to it.
   */
  _add_mobile_money_button_vuetify: function () {
    ...
    // ── 1. Find any existing payment-mode button ──
    let recCashBtn = mobile_payments.pos._find_payment_mode_button();

    if (!recCashBtn) {
      // Payment panel not open yet — polling / _hook_pay_button will retry.
      return;
    }

    mobile_payments.pos._log("Found payment-mode button:", recCashBtn.textContent.trim());

    // Verify button is actually visible (payment panel is open)
    ...
    if (!paymentRow) {
      mobile_payments.pos._log("No parent row found — using fallback");
      mobile_payments.pos._create_fallback_row();
      return;
    }

    mobile_payments.pos._log("Cloning payment row:", paymentRow.className);
    ...
    if (clonedBtn) {
      ...
      // Click → open payment dialog with current input amount
      freshBtn.addEventListener("click", function (e) {
        e.preventDefault();
        e.stopPropagation();
        let amountInput = document.querySelector("#mobile-payment-amount-input");
        // If input is empty/zero, refresh from the source row or grand total
        let currentVal = amountInput ? parseFloat(amountInput.value) || 0 : 0;
        if (!currentVal) {
          let sourceInput = paymentRow ? paymentRow.querySelector("input") : null;
          let refreshed = sourceInput ? (parseFloat(sourceInput.value) || 0) : 0;
          if (!refreshed) refreshed = mobile_payments.pos._get_grand_total_from_summary();
          if (amountInput && refreshed > 0) amountInput.value = refreshed.toFixed(2);
        }
    ...
    if (clonedInput) {
      ...
      // Pre-fill from the source REC CASH input (it already holds the grand total)
      let sourceInput = paymentRow ? paymentRow.querySelector("input") : null;
      let prefillVal = sourceInput ? (parseFloat(sourceInput.value) || 0) : 0;
      if (!prefillVal) prefillVal = mobile_payments.pos._get_grand_total_from_summary();
      freshInput.value = prefillVal > 0 ? prefillVal.toFixed(2) : "";
    ...
    for (let lbl of labels) {
      ...
      if (knownMopLabels.indexOf(text) !== -1) {
    ...
    mobile_payments.pos._log("MODE OF PAYMENT button injected successfully");
  ...
  _create_fallback_row: function () {
    ...
    // Wire up input — pre-fill with grand total immediately
    let input = row.querySelector("#mobile-payment-amount-input");
    if (input) {
      let prefill = mobile_payments.pos._get_grand_total_from_summary();
      input.value = prefill > 0 ? prefill.toFixed(2) : "";
    ...
    mobile_payments.pos._log("MODE OF PAYMENT fallback row injected");
  ...
  _attach_other_button_listeners: function () {
    ...
    document.addEventListener("click", function (e) {
      ...
      // Also ignore if this is the mobile payment amount input
      if (e.target.id === "mobile-payment-amount-input") return;
  ...
  /**
   * Read the Grand Total directly from the POS payment summary section.
   * More reliable than broad DOM scraping — targets only the specific
   * summary rows that display the final payable amount.
   */
  _get_grand_total_from_summary: function () {
    try {
      // Strategy 1: read from the REC CASH / first payment mode input in the panel
      // This is the most reliable — POS pre-fills it with the grand total
      let paymentInputs = document.querySelectorAll(
        ".v-row input[type=text], .v-row input[type=number], .v-row input:not([type])"
      );
      for (let inp of paymentInputs) {
        if (inp.id === "mobile-payment-amount-input") continue;
        let val = parseFloat(inp.value);
        if (val > 0) return val;
      }

      // Strategy 2: "Grand Total" label + adjacent value in the summary list
      let summaryRows = document.querySelectorAll(".v-list-item, .v-row");
      for (let row of summaryRows) {
        let cells = row.querySelectorAll(".v-list-item-title, .v-col, span, div");
        let foundLabel = false;
        for (let cell of cells) {
          let text = (cell.textContent || "").trim();
          if (/^grand\s*total$/i.test(text)) { foundLabel = true; continue; }
          if (foundLabel) {
      ...
      // Strategy 3: cur_frm (classic POS / Sales Invoice form)
      if (typeof cur_frm !== "undefined" && cur_frm && cur_frm.doc) {
        let v = flt(cur_frm.doc.rounded_total || cur_frm.doc.grand_total || 0);
        if (v > 0) return v;
      }
    } catch (e) {
      console.log("[MobilePayments] _get_grand_total_from_summary error:", e);
    }
    return 0;
  },
...
$(document).ready(function () {
  console.log("[MobilePayments POS] document.ready — URL:", window.location.href,
    "isPOS:", mobile_payments.pos._is_pos_page());
  ...
  setTimeout(function () {
    if (mobile_payments.pos._is_pos_page() && !mobile_payments.pos._initialized) {
      console.log("[MobilePayments POS] Retry init at 2s");
      mobile_payments.pos.init();
    }
  }, 2000);
  setTimeout(function () {
    if (mobile_payments.pos._is_pos_page() && !mobile_payments.pos._initialized) {
      console.log("[MobilePayments POS] Retry init at 5s");
      mobile_payments.pos.init();
    }
  }, 5000);
  setTimeout(function () {
    if (mobile_payments.pos._is_pos_page() && !mobile_payments.pos._initialized) {
      console.log("[MobilePayments POS] Retry init at 10s");
      mobile_payments.pos.init();
    }
  }, 10000);
});
