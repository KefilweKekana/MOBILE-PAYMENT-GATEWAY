...
mobile_payments.pos = {
  ...
  _update_submit_buttons: function () {
    ...
    document.querySelectorAll("button, .v-btn").forEach(function (btn) {
      ...
      if (alreadyVerified) {
        // Payment verified — hide the POS SUBMIT & PRINT button entirely
        // so users cannot double-submit (our code handles submit via API)
  ...
  /** Walk ALL Vue instances on the page and collect any matching store/state. */
  _walk_vue_stores: function () {
    var stores = [];
    try {
      // Collect __vue__ from all DOM elements (brute-force but reliable)
      var els = document.querySelectorAll("*");
      for (var i = 0; i < els.length; i++) {
        var vue = els[i].__vue__;
        if (!vue) continue;
        if (vue.$store && vue.$store.state && stores.indexOf(vue.$store.state) === -1) {
          stores.push(vue.$store.state);
        }
        // Also check direct component data
        if (vue._data && stores.indexOf(vue._data) === -1) {
          stores.push(vue._data);
        }
      }
    } catch (e) {}
    return stores;
  ...
  /** Try to get customer phone directly from POS Awesome's Vue store or invoice doc. */
  _get_customer_phone_from_vue: function () {
    try {
      ...
      // 2. Walk all Vue stores
      var stores = mobile_payments.pos._walk_vue_stores();
      for (var s = 0; s < stores.length; s++) {
        var st = stores[s];
        if (!st) continue;

        // Direct phone fields on state
  ...
  _get_customer: function () {
    try {
      ...
      // Walk all Vue stores for customer name
      var stores = mobile_payments.pos._walk_vue_stores();
      for (var s = 0; s < stores.length; s++) {
        var st = stores[s];
        if (!st) continue;
        ...
        if (st.customer_info && st.customer_info.name) return st.customer_info.name;
