...
// (guards against mobile_payment.js overwriting us later)
...
mobile_payments.pos = {
  ...
  _payment_panel_open: function () {
    // Detect the payment panel by the presence of ANY visible payment row
    // (a .v-row that contains both a button and an input).
    // This is more reliable than looking for a specific "CASH" label which
    // varies by locale and POS Awesome version.
    var rows = mobile_payments.pos._get_payment_rows();
    if (rows.length > 0) return true;

    // Fallback: look for any visible button whose label matches a known cash
    // variant — case-insensitive and partial match.
  ...
  _get_outstanding: function () {
    // Read the outstanding/total amount from multiple sources, most reliable first.

    // 1. Any non-zero payment row input (cash row is pre-filled by POS Awesome)
    var rows = mobile_payments.pos._get_payment_rows();
    var cashRe = /cash/i;
    // Prefer cash row first
    ...
    // 3. cur_frm fallback (standard Frappe forms)
    try {
    ...
    // 4. Read from visible total display element in POS UI
    try {
      var totalSelectors = [
  ...
  init: function () {
    ...
    // Always attach interceptors on the POS page — even if the API call returned
    // disabled, so the guard doesn't block navigation. If _enabled is false, the
    // _is_mobile_mop check inside the click handler will return false for everything
    // and no dialog will show. This prevents the early-return from swallowing the
    // interceptors when the API call is still in-flight or fails transiently.
  ...
  //
  // Problem: POS Awesome's capture listener on `document` loads BEFORE our
  // script, so our document-level stopImmediatePropagation() fires too late.
  //
  // Fix: bind directly ON each mobile MOP button element via MutationObserver.
  // Element-level listeners always beat document-level listeners in the same
  // phase regardless of registration order.
  _attach_interceptor: function () {
    ...
    function _handle_mobile_btn(e) {
      var btn = e.currentTarget;
      var lbl = (btn.textContent || "").trim().toUpperCase();
      if (!mobile_payments.pos._is_mobile_mop(lbl)) return;
      ...
      var methodObj = mobile_payments.pos._method_from_label(lbl);
      mobile_payments.pos.show_dialog(methodObj, outstanding, btn);
    }

    function _bind_btn(btn) {
      if (_bound.has(btn)) return;
      var label = (btn.textContent || "").trim().toUpperCase();
      if (!mobile_payments.pos._is_mobile_mop(label)) return;
      _bound.add(btn);
      // Bind in capture phase directly on the element — beats document listeners
      btn.addEventListener("click", _handle_mobile_btn, true);
    }

    function _scan_and_bind() {
      document.querySelectorAll("button, .v-btn").forEach(_bind_btn);
    }

    // Initial scans — Vue may not have rendered payment buttons yet
    setTimeout(_scan_and_bind, 500);
    setTimeout(_scan_and_bind, 1500);
    setTimeout(_scan_and_bind, 3000);

    // Re-scan whenever DOM changes (POS Awesome re-renders on cart updates)
    var _mo_timer = null;
    new MutationObserver(function (mutations) {
      var hasNew = mutations.some(function (m) { return m.addedNodes.length > 0; });
      if (!hasNew) return;
      clearTimeout(_mo_timer);
      _mo_timer = setTimeout(_scan_and_bind, 100);
...
Merchant Account",
      ...
      primary_action: function (values) {
        ...
        // Merchant till numbers are short numeric codes (no minimum length enforced here —
        // WaafiPay validates on their end). Subscriber numbers must be 7-15 digits.
        ...
        // Now safe to update payment row inputs — dialog is already closed
        // so Vue's re-render won't destroy anything.
        if (clickedBtn) {
          mobile_payments.pos._apply_single_selection(clickedBtn, values.amount);
        }
        mobile_payments.pos._update_submit_buttons();
...
$(document).ready(function () {
  // Defensive: ensure namespace survived any late overwrites
...
var _mp_nav_handler = function () {
  setTimeout(function () {
    // Defensive: ensure .pos exists (mobile_payment.js may have overwritten the global)
    if (!window.mobile_payments) window.mobile_payments = {};
