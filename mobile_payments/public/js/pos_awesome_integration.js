...
mobile_payments.pos = {
    ...
    var d = new frappe.ui.Dialog({
      ...
      fields: [
        ...
        },
        {
          ...
          default: amount,
          read_only: 1,
          description: __("Edit amount on the POS payment panel (left side) before opening this dialog."),
        },
      ],
  ...
  _show_success_dialog: function (amount, method) {
    var currency  = mobile_payments.pos._get_currency();
    var countdown = 0.5;   // seconds before auto-submit
    ...
    // ── Countdown ticker (0.5s) ──
    var remaining = countdown;
    var step = 0.5;  // tick every 500ms
    d._mp_timer = setInterval(function () {
      remaining -= step;

      // Update bar width
      var pct = Math.max(0, (remaining / countdown) * 100);
  ...
  _delegate_submit_print: function () {
    ...
    if (pos_doc && pos_doc.name) {
      console.log("mobile_payments.pos: found invoice via Vue store:", pos_doc.name);
      mobile_payments.pos._submit_invoice(pos_doc.name, pos_doc.doctype || "POS Invoice");
      return;
    }

    // Fallback: ask the server for the current draft POS Invoice
    console.log("mobile_payments.pos: Vue store failed, trying server fallback…");
    frappe.call({
      method: "mobile_payments.api.pos.get_current_draft_pos_invoice",
      args: {
        pos_profile: mobile_payments.pos._get_pos_profile() || "",
        customer: mobile_payments.pos._get_customer() || "",
      },
      callback: function (r) {
        var result = r && r.message;
        if (result && result.found && result.name) {
          console.log("mobile_payments.pos: found invoice via server:", result.name);
          mobile_payments.pos._submit_invoice(result.name, result.doctype || "POS Invoice");
        } else {
          // All methods failed — unstick: stop enforcer, re-enable buttons
          console.error("mobile_payments.pos: could not find invoice via Vue or server");
  ...
  /** Fetch fresh doc, fix payment mode, submit, and open print */
  _submit_invoice: function (invoice_name, doctype) {
    frappe.call({
      method: "frappe.client.get",
      args: { doctype: doctype, name: invoice_name },
      callback: function (r) {
  ...
  _get_pos_doc: function () {
    try {
      // Helper: extract invoice_doc from a Vue instance or its store
      function _extract(vue) {
        if (!vue) return null;
        // Vuex store
        if (vue.$store && vue.$store.state) {
          var st = vue.$store.state;
          if (st.invoice_doc && st.invoice_doc.name) return st.invoice_doc;
        }
        // Direct data props
        if (vue.invoice_doc && vue.invoice_doc.name) return vue.invoice_doc;
        if (vue.invoiceDoc && vue.invoiceDoc.name) return vue.invoiceDoc;
        return null;
      }

      // Helper: get Vue instance from a DOM element (Vue 2 or Vue 3)
      function _getVue(el) {
        if (!el) return null;
        // Vue 2
        if (el.__vue__) return el.__vue__;
        // Vue 3 — __vue_app__ exposes the app, dig into the root component
        if (el.__vue_app__) {
          var app = el.__vue_app__;
          // Try the app's global store (Vuex / Pinia)
          if (app.config && app.config.globalProperties && app.config.globalProperties.$store) {
            ...
            if (st && st.invoice_doc && st.invoice_doc.name) return { $store: { state: st } };
          }
          // Try root component proxy
          if (app._instance && app._instance.proxy) return app._instance.proxy;
        }
        return null;
      }

      // 1. Try known POS Awesome mount points
      var selectors = [
        "[data-app]", "#app", "#pos-awesome-root", ".pos-awesome-app",
        "[class*='posa']", ".v-application", "[id^='posa']",
      ];
      for (var s = 0; s < selectors.length; s++) {
        var el = document.querySelector(selectors[s]);
        var vue = _getVue(el);
        var doc = _extract(vue);
        if (doc) {
          console.log("mobile_payments: found invoice_doc via selector", selectors[s], doc.name);
          return doc;
        }
        // Walk children (Vue 2 $children tree)
        if (vue && vue.$children) {
          var queue = vue.$children.slice();
          var depth = 0;
          while (queue.length && depth < 500) {
            ...
            doc = _extract(child);
            if (doc) {
              console.log("mobile_payments: found invoice_doc via child walk, depth", depth, doc.name);
              return doc;
            }
            if (child.$children) {
      ...
      // 2. Brute-force: walk all DOM elements
      var allEls = document.querySelectorAll("*");
      for (var i = 0; i < allEls.length; i++) {
        var vue2 = _getVue(allEls[i]);
        var doc2 = _extract(vue2);
        if (doc2) {
          console.log("mobile_payments: found invoice_doc via DOM walk on", allEls[i].tagName, doc2.name);
          return doc2;
        }
      }

      // 3. Log what we DID find for debugging
      for (var j = 0; j < allEls.length; j++) {
        var v = _getVue(allEls[j]);
        if (v && v.$store && v.$store.state) {
          console.log("mobile_payments: store found with keys:", Object.keys(v.$store.state));
          break;
        }
      }
    } catch (e) {
  ...
  _walk_vue_stores: function () {
    ...
    var datas  = [];
    try {
      // Helper: get Vue instance from element (Vue 2 or Vue 3)
      function _getVue(el) {
        if (!el) return null;
        if (el.__vue__) return el.__vue__;
        if (el.__vue_app__) {
