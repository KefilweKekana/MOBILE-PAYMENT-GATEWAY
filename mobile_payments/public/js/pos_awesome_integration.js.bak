...
mobile_payments.pos = {
  ...
  _on_success: function (result, amount, method) {
    ...
    // Let POS Awesome handle final submit flow: click SUBMIT & PRINT automatically.
    setTimeout(function () {
      mobile_payments.pos._confirm_mop_amount(amount);
      mobile_payments.pos._auto_submit_with_retry(8, 500); // retry up to 8 times
    }, 500);
  },

  _auto_submit_with_retry: function (maxTries, delayMs) {
    var tries = 0;
    var timer = setInterval(function () {
      tries++;
      var clicked = mobile_payments.pos._auto_submit();
      if (clicked || tries >= (maxTries || 8)) {
        clearInterval(timer);
        if (!clicked) {
          frappe.show_alert({
  ...
  /** Programmatically click SUBMIT & PRINT (preferred) after verified payment. */
  _auto_submit: function () {
    if (!mobile_payments.pos._payment_verified) return false;
    ...
    // Native click + MouseEvent for Vue listeners
    target.click();
