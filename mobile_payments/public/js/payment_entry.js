...
;(function () {
  console.log("[MobilePayments] payment_entry.js loaded");
  ...
  frappe.ui.form.on("Payment Entry", {
    refresh: function (frm) {
      console.log("[MobilePayments] refresh fired, mode_of_payment:", frm.doc.mode_of_payment, "docstatus:", frm.doc.docstatus);
      add_pay_button(frm);
    ...
    before_submit: function (frm) {
      console.log("[MobilePayments] before_submit fired, mode:", frm.doc.mode_of_payment, "verified:", _payment_verified);
  ...
  function add_pay_button(frm) {
    ...
    if (!is_mobile_mode(frm.doc.mode_of_payment)) {
      console.log("[MobilePayments] add_pay_button skipped - not mobile mode:", frm.doc.mode_of_payment);
      return;
    }
    ...
    // V15 has change_custom_button_type; V14 does not
    if (frm.change_custom_button_type) {
      frm.change_custom_button_type(__("Pay with Mobile"), null, "primary");
    } else if (frm.custom_buttons && frm.custom_buttons[__("Pay with Mobile")]) {
      frm.custom_buttons[__("Pay with Mobile")].removeClass("btn-default").addClass("btn-primary");
    }

    // Add "Generate Payment Links" button if links are missing
    if (!frm.is_new() && (!frm.doc.waafi_payment_link || !frm.doc.edahab_payment_link)) {
      frm.add_custom_button(__("Generate Payment Links"), function () {
        frappe.call({
          method: "mobile_payments.utils.payment_links.generate_pe_links_manual",
          ...
          callback: function (r) {
            if (r.message) {
              var parts = [];
              if (r.message.waafi_payment_link) parts.push("WaafiPay");
              if (r.message.edahab_payment_link) parts.push("Edahab");
              if (parts.length) {
                frappe.show_alert({ message: __("Payment links generated: {0}", [parts.join(", ")]), indicator: "green" }, 5);
              } else {
                frappe.show_alert({ message: __("No links generated. Check Error Log."), indicator: "orange" }, 5);
              }
            }
            frm.reload_doc();
          },
    ...
    frappe.call({
      ...
      callback: function (r) {
        ...
        if (result.success) {
          ...
          // Save payment details to the PE custom fields.
          // Skip if server already finalized (result.payment_entry means
          // _finalize_payment_entry already set fields + submitted the PE).
  ...
  function submit_payment_entry(frm) {
    ...
    frm.reload_doc(function () {
      // Server may have already submitted the PE via _finalize_payment_entry
