...
def generate_pe_payment_links(doc, method=None):
    """
    Hook for Payment Entry after_insert.
    When a PE is saved with a mobile Mode of Payment, auto-generate
    WaafiPay and Edahab payment links.
    """
    if doc.docstatus != 0:
        return
    ...
    # Check that custom fields exist in DB — auto-create if missing
    pe_meta = frappe.get_meta("Payment Entry")
    if not pe_meta.has_field("waafi_payment_link"):
        try:
            from mobile_payments.install import _create_custom_fields
...
def _generate_links_for_payment_entry(doc):
    ...
    Creates token-based URLs that can be shared with the payer.
    """
    import secrets
    ...
    # Phone number is required for transaction log
