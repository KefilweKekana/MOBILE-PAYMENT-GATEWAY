...
def generate_payment_links_if_due(doc, method=None):
    ...
    Generates payment links for submitted, unpaid invoices.
    """
    ...
    if result:
        ...
        if update_fields:
            frappe.db.set_value("Sales Invoice", doc.name, update_fields,
                                update_modified=False)
            # Update in-memory doc so the form response includes the links
...
def generate_pe_payment_links(doc, method=None):
    ...
    When a PE is saved with a mobile Mode of Payment, generate
    payment links inline.
    """
    if doc.docstatus != 0:
        return
    ...
    try:
        ...
        frappe.log_error(
            message=f"PE payment link generation failed for {doc.name}: {str(e)}\n{frappe.get_traceback()}",
...
def _generate_links_for_payment_entry(doc):
    ...
    # Provider-specific phone lookups
    waafi_phone = customer_phone
    edahab_phone = customer_phone
    if customer:
        try:
            from mobile_payments.api.pos import get_customer_phone_for_provider
            waafi_data = get_customer_phone_for_provider(customer, provider="WaafiPay")
            if waafi_data and waafi_data.get("phone"):
                waafi_phone = waafi_data["phone"]
            edahab_data = get_customer_phone_for_provider(customer, provider="Edahab")
            if edahab_data and edahab_data.get("phone"):
                edahab_phone = edahab_data["phone"]
        except Exception:
            pass
