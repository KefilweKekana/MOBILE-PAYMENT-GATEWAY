...
def _generate_links_for_invoice(doc):
    ...
    if settings.waafipay_enabled and not doc.get("waafi_payment_link"):
        try:
            # Validate credentials before attempting link generation
            creds = settings.get_waafipay_credentials()
            if not creds.get("merchant_uid"):
                frappe.log_error(
                    message=f"WaafiPay Merchant UID is missing. Cannot generate payment link for {doc.name}.",
                    title="Payment Link - Missing Credentials",
                )
                result["waafi_payment_link"] = ""
            elif not creds.get("api_key"):
                frappe.log_error(
                    message=f"WaafiPay API Key is missing. Cannot generate payment link for {doc.name}.",
                    title="Payment Link - Missing Credentials",
                )
                result["waafi_payment_link"] = ""
            else:
    ...
    if settings.edahab_enabled and not doc.get("edahab_payment_link"):
        try:
            ...
            if not edahab_creds.get("api_key"):
                frappe.log_error(
                    message=f"Edahab API Key is missing. Cannot generate payment link for {doc.name}.",
                    title="Payment Link - Missing Credentials",
                )
                result["edahab_payment_link"] = ""
            elif not edahab_creds.get("api_secret"):
                frappe.log_error(
                    message=f"Edahab API Secret is missing. Cannot generate payment link for {doc.name}.",
                    title="Payment Link - Missing Credentials",
                )
                result["edahab_payment_link"] = ""
            else:
