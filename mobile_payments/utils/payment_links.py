...
def invalidate_payment_links_on_cancel(doc, method=None):
    ...
    """
    # ── 1. Clear the persistent-link fields on the invoice ──
    update_fields = {}
    if doc.get("waafi_payment_link"):
        update_fields["waafi_payment_link"] = ""
    if doc.get("edahab_payment_link"):
        update_fields["edahab_payment_link"] = ""

    if update_fields:
        frappe.db.set_value(
            "Sales Invoice", doc.name, update_fields,
            update_modified=False,
        )

    # ── 2. Cancel all active payment-link tokens for this invoice ──
