...
def get_dashboard_data(from_date=None, to_date=None):
    ...
    # Successful amount breakdown by currency
    currency_totals = frappe.db.sql(
        """
        SELECT IFNULL(currency, 'USD') as currency, COALESCE(SUM(amount), 0) as total_amount
        FROM `tabMobile Payment Transaction Log`
        WHERE status = 'Completed'
          AND DATE(initiated_at) BETWEEN %s AND %s
        GROUP BY IFNULL(currency, 'USD')
        ORDER BY currency
        """,
        (from_date, to_date),
        as_dict=True,
    ...
    # Provider breakdown (grouped by currency)
    provider_stats = frappe.db.sql(
        ...
        ORDER BY provider, payment_method, currency
        """,
        (from_date, to_date),
        as_dict=True,
    )
