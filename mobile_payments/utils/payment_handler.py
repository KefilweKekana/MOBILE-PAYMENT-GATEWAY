...
def on_sales_invoice_submit(doc, method):
    ...
    """
    try:
        # Look for a completed transaction log linked to this invoice
        logs = frappe.get_all(
            "Mobile Payment Transaction Log",
            filters={
        ...
        # Update invoice fields directly — invoice is now submitted (docstatus=1)
        frappe.db.set_value(
            doc.doctype,
            doc.name,
            {
        ...
        frappe.logger("mobile_payments").info(
            f"on_sales_invoice_submit: updated mobile payment fields on {doc.name} "
            f"from log {log_data.name}"
        )

        # Enqueue full payment processing (creates Payment Entry etc.)
        frappe.enqueue(
            process_successful_payment,
            queue="short",
