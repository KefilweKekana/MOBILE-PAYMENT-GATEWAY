...
def process_successful_payment(transaction_log, invoice_id=None):
    ...
    try:
        ...
        if log.payment_entry:
            # The PE may have been pre-linked by initiate_payment_entry_mobile_payment
            # while the payment was still pending.  Now that it's Completed, ensure
            # reference_no carries the provider's transaction ID.
            if log.provider_transaction_id:
                try:
                    pe_ref = frappe.db.get_value(
                        "Payment Entry", log.payment_entry, "reference_no",
                    )
                    if not pe_ref or pe_ref == log.name:
                        frappe.db.set_value("Payment Entry", log.payment_entry, {
                            "reference_no": log.provider_transaction_id,
                            "mobile_payment_reference": log.provider_transaction_id,
...
# ──────────────────────────────────────────────
# Payment Entry – Mobile Money Integration
# ──────────────────────────────────────────────

@frappe.whitelist()
def initiate_payment_entry_mobile_payment(payment_entry, phone, provider="WaafiPay",
                                           method="ZAAD"):
    """
    Initiate a mobile USSD-push payment for an *existing* draft Payment Entry
    whose Mode of Payment is "Mobile Money" (or similar).

    Unlike the Sales Invoice flow, this does NOT auto-create a Payment Entry —
    the user already has one in draft.  On success we:
      1. stamp ``reference_no``, ``mobile_payment_reference``, and
         ``mobile_payment_transaction_id`` on the PE so the operator can review
         and submit it;
      2. link the Mobile Payment Transaction Log back to the PE.

    Args:
        payment_entry: Draft Payment Entry name
        phone: Customer mobile-wallet number
        provider: WaafiPay | Edahab
        method: ZAAD | SAHAL | EVCPlus | Edahab

    Returns:
        dict – same shape as ``initiate_waafipay_payment`` / ``initiate_edahab_payment``
        so the frontend can reuse the existing polling / success / error UI.
    """
    frappe.has_permission("Payment Entry", "write", throw=True)

    pe = frappe.get_doc("Payment Entry", payment_entry)
    if pe.docstatus != 0:
        frappe.throw(_("Payment Entry must be in Draft to initiate a mobile payment"))

    amount = flt(pe.paid_amount)
    if amount <= 0:
        frappe.throw(_("Paid amount must be greater than zero"))

    currency = pe.paid_to_account_currency or pe.paid_from_account_currency or "USD"
    currency = _normalize_currency(currency)

    # Resolve the Sales Invoice from PE references (if any)
    invoice_id = None
    for ref in (pe.references or []):
        if ref.reference_doctype == "Sales Invoice" and ref.reference_name:
            invoice_id = ref.reference_name
            break

    description = f"Payment Entry {pe.name}"
    if invoice_id:
        description += f" for {invoice_id}"
    ...
    # ── On success: stamp the PE, link the transaction log ──
    log_name = result.get("transaction_log")
    if result.get("success") and log_name:
        provider_ref = result.get("provider_transaction_id") or log_name
        try:
            frappe.db.set_value("Payment Entry", pe.name, {
                "reference_no": provider_ref,
                "reference_date": getdate(now_datetime()),
                "mobile_payment_reference": provider_ref,
                "mobile_payment_transaction_id": log_name,
            }, update_modified=False)

            # Link the transaction log back to the Payment Entry
            frappe.db.set_value("Mobile Payment Transaction Log", log_name, {
            ...
            frappe.log_error(
                ...
                title=f"PE stamp error for {pe.name}",
            )
    elif result.get("pending") and log_name:
        # Payment is pending — link the log now; reference_no will be set when
        # the poll/callback marks it Completed.
        try:
            frappe.db.set_value("Payment Entry", pe.name, {
                "mobile_payment_transaction_id": log_name,
            }, update_modified=False)
    ...
    # Return same shape for the frontend polling UI
