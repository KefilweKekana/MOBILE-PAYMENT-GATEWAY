...
def _create_paid_invoice(log, invoice_id, settings):
    ...
    try:
        logger.info(
            ...
            f"| user={frappe.session.user} | ignore_permissions={getattr(frappe.flags, 'ignore_permissions', False)}"
        )

        invoice = frappe.get_doc("Sales Invoice", invoice_id)
        logger.info(
            f"[_create_paid_invoice] Loaded invoice {invoice_id} | docstatus={invoice.docstatus} "
            f"| outstanding={invoice.outstanding_amount} | currency={invoice.currency} "
            f"| grand_total={invoice.grand_total} | company={invoice.company}"
        )
        ...
        logger.info(
            f"[_create_paid_invoice] MOP={mode_of_payment} | payment_account={payment_account}"
        )
        ...
        logger.info(
            f"[_create_paid_invoice] Amounts: payment={payment_amount} "
            f"| received={received_amount} | outstanding={outstanding} "
            f"| exchange_rate={source_exchange_rate}"
        )

        # ── Step 5: Build and submit the Payment Entry ──────────────────────
        remarks = (
        ...
        logger.info(
            ...
            f"| {mode_of_payment} | Amount: {payment_amount} {paid_from_currency}"
        )
        ...
        frappe.log_error(
            message=(
                f"CRITICAL: Failed to create Payment Entry for {invoice_id}\n"
                f"Error: {str(e)}\n"
                f"User: {frappe.session.user}\n"
                f"ignore_permissions: {getattr(frappe.flags, 'ignore_permissions', False)}\n"
