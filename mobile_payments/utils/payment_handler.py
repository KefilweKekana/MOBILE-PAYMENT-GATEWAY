...
def process_successful_payment(transaction_log, invoice_id=None):
    ...
    or inline from HPP return handlers (which may run as Guest).
    ...
    """
    # HPP return callbacks and payment-link endpoints run as Guest.
    # We need full permissions to read invoices and create Payment Entries.
    # Belt-and-suspenders: set_user("Administrator") AND ignore_permissions flag.
    original_user = frappe.session.user
    original_ignore = getattr(frappe.flags, 'ignore_permissions', False)
    needs_escalation = (
        original_user == "Guest"
        or not frappe.has_permission("Sales Invoice", "read", throw=False)
    )
    if needs_escalation:
        frappe.set_user("Administrator")
    # Always set ignore_permissions for payment processing — this is a system
    # operation that must not be blocked by role restrictions.
    ...
    try:
        ...
        # Already processed?
        # For SI-based logs: payment_entry is set AFTER PE is created, so
        # its presence means we're done.
        # For PE-based logs: payment_entry is set from the start (the PE
        # existed before payment), so check if it's already submitted.
        if log.payment_entry:
            pe_docstatus = frappe.db.get_value("Payment Entry", log.payment_entry, "docstatus")
            if pe_docstatus == 1:
                frappe.logger("mobile_payments").info(
                    f"Payment Entry already submitted for {log.name}: {log.payment_entry}"
                )
                return
            # PE exists but is still Draft — finalize it (update fields + submit)
            _finalize_payment_entry(log, log.payment_entry)
            frappe.db.commit()
            return
        ...
        if not invoice_id:
            frappe.logger("mobile_payments").warning(
                f"No Sales Invoice or Payment Entry linked to transaction {log.name}"
            )
            return

        # Guard: invoice_id might be a PE name from old logs — skip SI flow
        if invoice_id and not frappe.db.exists("Sales Invoice", invoice_id):
            if frappe.db.exists("Payment Entry", invoice_id):
                log.db_set("payment_entry", invoice_id)
                log.payment_entry = invoice_id
                if log.sales_invoice:
                    log.db_set("sales_invoice", None)
                _finalize_payment_entry(log, invoice_id)
                frappe.db.commit()
                return
            frappe.logger("mobile_payments").warning(
                f"invoice_id {invoice_id} is not a Sales Invoice or Payment Entry"
            )
            return
        ...
        if result:
            ...
            # Write error to log so it's visible on the transaction record
            log.db_set("error_message",
                f"PE creation returned None — check Error Log for details",
        ...
        # Write error to transaction log for visibility
        try:
            frappe.db.set_value("Mobile Payment Transaction Log", transaction_log,
                "error_message", f"PE Error: {str(e)}", update_modified=False)
            frappe.db.commit()
        except Exception:
            pass
        ...
        if needs_escalation and frappe.session.user != original_user:
            try:
...
def _create_paid_invoice(log, invoice_id, settings):
    """
    Create a Payment Entry for a completed mobile payment.

    Uses ERPNext's built-in ``get_payment_entry`` helper which handles all
    currency, exchange-rate, and account mappings automatically — exactly
    the same as clicking "Payment > Payment Entry" in the UI.
    ...
    """
    logger = frappe.logger("mobile_payments")
    try:
        logger.info(
            f"[_create_paid_invoice] START | log={log.name} | invoice={invoice_id} "
            f"| log_status={log.status} | user={frappe.session.user}"
        )

        if log.status != "Completed":
            logger.warning(f"Transaction {log.name} status is {log.status}, not Completed — skipping")
            return None

        if not frappe.db.exists("Sales Invoice", invoice_id):
            logger.warning(f"Sales Invoice {invoice_id} does not exist")
            return None

        invoice = frappe.get_doc("Sales Invoice", invoice_id)
        logger.info(
            f"[_create_paid_invoice] invoice docstatus={invoice.docstatus} "
            f"outstanding={invoice.outstanding_amount} currency={invoice.currency}"
        )

        if invoice.docstatus == 2:
            logger.warning(f"Sales Invoice {invoice_id} is cancelled — skipping")
            return None

        # Submit draft invoices first
        if invoice.docstatus == 0:
            try:
                ...
                frappe.db.commit()
                invoice.reload()
                logger.info(f"Submitted draft Sales Invoice {invoice_id}")
            except Exception as e:
                frappe.log_error(
                    message=f"Failed to submit draft Sales Invoice {invoice_id}: {str(e)}\n{frappe.get_traceback()}",
        ...
        if outstanding <= 0:
            logger.info(f"Sales Invoice {invoice_id} already fully paid (outstanding={outstanding})")
            return None

        # ── Use ERPNext's get_payment_entry to build PE automatically ───────
        from erpnext.accounts.doctype.payment_entry.payment_entry import get_payment_entry

        pe = get_payment_entry("Sales Invoice", invoice_id)
        logger.info(
            f"[_create_paid_invoice] get_payment_entry returned PE | "
            f"paid_amount={pe.paid_amount} received_amount={pe.received_amount} "
            f"paid_from={pe.paid_from} paid_to={pe.paid_to}"
        )

        # Override mode of payment if we have one
        mode_of_payment = _get_mode_of_payment(log.payment_method or log.provider)
        if mode_of_payment:
            pe.mode_of_payment = mode_of_payment
            # Update paid_to account from Mode of Payment if available
            ...
            if mop_account:
                ...
                pe.paid_to_account_currency = frappe.db.get_value("Account", mop_account, "account_currency")

        pe.reference_no = log.provider_transaction_id or log.name
        ...
        logger.info(f"[_create_paid_invoice] Inserting PE for {invoice_id}...")
        pe.insert(ignore_permissions=True)
        logger.info(f"[_create_paid_invoice] Submitting PE {pe.name}...")
        pe.submit()
        frappe.db.commit()

        logger.info(
            f"Payment Entry {pe.name} created for {invoice_id} | "
            f"amount={pe.paid_amount} | provider={log.provider}"
        )
        ...
        frappe.log_error(
            message=(
                ...
                f"Log: {log.name if log else 'N/A'}\n"
                f"Provider: {log.provider if log else 'N/A'}\n"
                f"Amount: {log.amount if log else 'N/A'}\n"
                f"User: {frappe.session.user}\n"
                f"{frappe.get_traceback()}"
            ),
            title=f"PE Creation Failed: {invoice_id}",
        )
        return None
...
def _finalize_payment_entry(log, pe_name):
    """
    Finalize a Payment Entry after successful HPP/link payment.

    The PE already exists (created manually or via link generation).
    We just need to:
      1. Update its mobile payment custom fields
      2. Auto-submit it if still in Draft
    ...
    try:
        ...
        field_updates = {}
        if pe_meta.has_field("mobile_payment_status"):
            field_updates["mobile_payment_status"] = "Completed"
        if pe_meta.has_field("mobile_payment_provider"):
            field_updates["mobile_payment_provider"] = log.provider
        if pe_meta.has_field("mobile_payment_method"):
            field_updates["mobile_payment_method"] = log.payment_method
        if pe_meta.has_field("mobile_payment_phone"):
            field_updates["mobile_payment_phone"] = log.phone_number if log.phone_number != "HPP" else ""
        if pe_meta.has_field("mobile_payment_reference"):
            field_updates["mobile_payment_reference"] = log.provider_transaction_id or log.name
        if pe_meta.has_field("mobile_payment_transaction_id"):
            field_updates["mobile_payment_transaction_id"] = log.name

        if field_updates:
            frappe.db.set_value("Payment Entry", pe_name, field_updates, update_modified=False)

        # Update reference_no if empty
        ...
        if pe.docstatus == 0:
            ...
            pe.flags.ignore_permissions = True
            pe.submit()
            logger.info(f"Auto-submitted Payment Entry {pe_name} after HPP payment | log={log.name}")
        else:
            logger.info(f"Payment Entry {pe_name} already submitted (docstatus={pe.docstatus})")

        # Link the PE on the transaction log
        ...
        frappe.log_error(
            message=(
                f"Failed to finalize Payment Entry {pe_name}: {str(e)}\n"
                f"Log: {log.name}\n{frappe.get_traceback()}"
            ),
            title=f"PE Finalize Failed: {pe_name}",
        )
...
def _get_mode_of_payment(method):
    ...
    frappe.logger("mobile_payments").warning(
        f"Mode of Payment '{mode}' not found in ERPNext — PE will use default account"
    )
    return None
...
# ──────────────────────────────────────────────
# Safety-net: Retry PE creation for completed logs
# ──────────────────────────────────────────────

def retry_pending_payment_entries():
    """
    Scheduled task: Find completed transaction logs that have no Payment Entry
    and attempt to create one. This catches any cases where the enqueued job
    failed or was never created.

    Should be called every 1-2 minutes via hooks.py scheduler_events.
    Runs in the scheduler worker context (Administrator).
    """
    logs = frappe.get_all(
    ...
    frappe.logger("mobile_payments").info(
        f"retry_pending_payment_entries: Found {len(logs)} completed logs without PE"
    )

    for log_row in logs:
        try:
            process_successful_payment(
                transaction_log=log_row.name,
                invoice_id=log_row.sales_invoice,
            )
        except Exception as e:
            frappe.log_error(
                message=f"Safety-net PE retry failed for {log_row.name}: {str(e)}\n{frappe.get_traceback()}",
                title=f"PE Retry Failed: {log_row.name}",
            )
...
def initiate_appointment_payment(phone, amount, appointment_name,
    ...
    if result.get("success"):
        ...
        # Mark as invoiced + set mode_of_payment so ERPNext hides
        # "Make Payment" / "Create Unpaid Invoice" buttons.
        apt_meta = frappe.get_meta("Patient Appointment")
        if apt_meta.has_field("invoiced"):
            apt_update["invoiced"] = 1
        if apt_meta.has_field("mode_of_payment"):
            mop = method if provider == "WaafiPay" else "Edahab"
            apt_update["mode_of_payment"] = _get_mode_of_payment(mop)
        frappe.db.set_value("Patient Appointment", appointment_name,
                            apt_update, update_modified=False)
...
def _create_appointment_invoice(appointment_name, provider, method,
    ...
    try:
        ...
        update_fields = {"mobile_payment_sales_invoice": si.name}
        # Also fill ERPNext's standard ref_sales_invoice so both stay in sync
        apt_meta = frappe.get_meta("Patient Appointment")
        if apt_meta.has_field("ref_sales_invoice"):
            update_fields["ref_sales_invoice"] = si.name
        # Mark as invoiced and set mode_of_payment so ERPNext's standard
        # "Make Payment" / "Create Unpaid Invoice" buttons disappear.
        if apt_meta.has_field("invoiced"):
            update_fields["invoiced"] = 1
        if apt_meta.has_field("mode_of_payment"):
            update_fields["mode_of_payment"] = _get_mode_of_payment(method or provider)
        frappe.db.set_value(
...
def process_successful_appointment_payment(transaction_log):
    ...
    try:
        ...
        if si:
            # Update appointment status + mark as invoiced
            apt_update = {
                "mobile_payment_status": "Completed",
            ...
            if apt_meta.has_field("invoiced"):
                apt_update["invoiced"] = 1
            if apt_meta.has_field("mode_of_payment"):
                apt_update["mode_of_payment"] = _get_mode_of_payment(
                    log.payment_method or log.provider
                )
