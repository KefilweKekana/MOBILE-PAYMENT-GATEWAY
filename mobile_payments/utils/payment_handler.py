...
def process_successful_payment(transaction_log, invoice_id=None):
    ...
    try:
        ...
        # Create a paid Sales Invoice (with payment inside) or Payment Entry
...
def _create_paid_invoice(log, invoice_id, settings):
    """
    Create a paid Sales Invoice with payment embedded inside it.
    For draft invoices: adds payment row, sets is_pos=1, and submits as paid.
    For already-submitted invoices: falls back to Payment Entry.
    ...
    Returns:
        dict with 'type' and 'reference' keys, or None
    """
    try:
        ...
        payment_amount = flt(log.amount) or flt(invoice.grand_total)

        # ── DRAFT invoice → add payment inside and submit as paid ──
        if invoice.docstatus == 0:
            try:
                # Enable POS-style payment inside the invoice
                invoice.is_pos = 1
                ...
                if not invoice.pos_profile:
                    pos_profile = frappe.db.get_value(
                        "POS Profile",
                        {"company": invoice.company, "disabled": 0},
                        "name"
                    )
                    if pos_profile:
                        invoice.pos_profile = pos_profile

                # Clear any existing payments and add the mobile payment
                ...
                invoice.base_paid_amount = payment_amount
                ...
                frappe.logger("mobile_payments").info(
                    f"Submitted paid Sales Invoice {invoice_id} with {mode_of_payment} payment"
                    f" | Amount: {payment_amount}"
                )

                return {"type": "Paid Sales Invoice", "reference": invoice_id}

            except Exception as e:
                frappe.log_error(
                    message=(
                        f"Failed to create paid Sales Invoice {invoice_id}: {str(e)}\n"
                        f"{frappe.get_traceback()}"
                    ),
                    title="Paid Invoice Error",
                )
                return None

        # ── SUBMITTED invoice → create Payment Entry (only way to pay already-submitted SI) ──
            ...
            frappe.logger("mobile_payments").warning(
                f"Sales Invoice {invoice_id} is cancelled (docstatus={invoice.docstatus})"
            )
            return None
