...
def _create_paid_invoice(log, invoice_id, settings):
    """
    Mark a Sales Invoice as paid when the transaction log status is Completed.

    Directly updates the Sales Invoice outstanding_amount and status via DB,
    bypassing Payment Entry creation entirely. This is a simple, reliable
    approach that avoids permission and currency issues with PE creation.
    ...
    try:
        logger.info(
            ...
            f"| log_status={log.status}"
        )

        if log.status != "Completed":
            logger.warning(
                f"Transaction {log.name} status is {log.status}, not Completed — skipping"
            )
            return None
        ...
        payment_amount = flt(log.amount) or outstanding
        ...
        logger.info(
            f"Sales Invoice {invoice_id} marked as Paid | amount={payment_amount} "
            f"| provider={log.provider} | ref={log.provider_transaction_id or log.name}"
        )

        return {"type": "Direct Update", "reference": log.name}

    except Exception as e:
        frappe.log_error(
            message=(
                f"Failed to mark invoice {invoice_id} as paid: {str(e)}\n"
                f"Log: {log.name if log else 'N/A'}\n"
                f"{frappe.get_traceback()}"
            ),
            title=f"Mark Paid Failed: {invoice_id}",
        )
        return None
