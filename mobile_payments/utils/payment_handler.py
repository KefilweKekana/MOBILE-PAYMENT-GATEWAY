...
def _create_payment_entry(log, invoice_id, settings):
    ...
    try:
        ...
        # Auto-submit draft Sales Invoice if payment is confirmed
        if invoice.docstatus == 0:
            try:
                invoice.submit()
                frappe.db.commit()
                frappe.logger("mobile_payments").info(
                    f"Auto-submitted Sales Invoice {invoice_id} after confirmed payment"
                )
            except Exception as e:
                frappe.log_error(
                    message=(
                        f"Failed to auto-submit Sales Invoice {invoice_id}: {str(e)}\n"
                        f"{frappe.get_traceback()}"
                    ),
                    title="Invoice Auto-Submit Error",
                )
                return None
