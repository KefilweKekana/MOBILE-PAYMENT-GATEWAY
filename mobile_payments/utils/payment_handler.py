...
def process_successful_payment(transaction_log, invoice_id=None):
    ...
    try:
        ...
        frappe.log_error(
            ...
            "Mobile Payment Processing Error",
        )
...
def _create_payment_entry(log, invoice_id, settings):
    ...
    try:
        ...
        frappe.log_error(
            ...
            f"{frappe.get_traceback()}",
            "Mobile Payment - Payment Entry Error",
        )
        return None
...
def _update_sales_invoice(log, invoice_id, settings):
    ...
    try:
        ...
        frappe.log_error(
            f"Failed to update Sales Invoice {invoice_id}: {str(e)}",
            "Mobile Payment - Invoice Update Error",
        )
...
def poll_pending_transactions():
    ...
    for txn in pending_logs:
        try:
            ...
            frappe.log_error(
                f"Error polling transaction {txn['name']}: {str(e)}",
                "Mobile Payment Polling Error",
            )
...
def _poll_waafipay(log):
    ...
    try:
        ...
        frappe.log_error(
            f"WaafiPay poll error for {log.name}: {str(e)}",
            "WaafiPay Polling Error",
        )
...
def _poll_edahab(log):
    ...
    try:
        ...
        frappe.log_error(
            f"Edahab poll error for {log.name}: {str(e)}",
            "Edahab Polling Error",
        )
...
def retry_failed_transactions():
    ...
    for txn in retryable:
        try:
            ...
            frappe.log_error(
                f"Error retrying transaction {txn['name']}: {str(e)}",
                "Mobile Payment Retry Error",
            )
...
def initiate_appointment_payment(phone, amount, appointment_name,
    ...
    if provider == "Edahab":
        ...
        result = client.purchase_request(
            phone=phone,
            amount=amount,
            invoice_id=appointment_name,
            description=description or f"Appointment payment - {appointment.patient_name or appointment_name}",
            currency=currency,
        )
    else:
        from mobile_payments.api.waafipay import WaafiPayClient
    ...
    # Update Patient Appointment with payment status
    if result.get("success"):
        frappe.db.set_value("Patient Appointment", appointment_name, {
            ...
            "mobile_payment_transaction_id": result.get("transaction_log", ""),
        }, update_modified=False)
