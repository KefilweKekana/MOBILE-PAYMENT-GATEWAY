...
def validate_ip_whitelist(request_ip):
    ...
    if request_ip not in allowed_ips:
        frappe.log_error(
            message=f"Blocked webhook from unauthorized IP: {request_ip}",
...
def validate_webhook_signature(payload, signature, provider="WaafiPay"):
    ...
    if not secret:
        frappe.log_error(
            message="Webhook validation enabled but no secret key configured",
            title="Mobile Payment Security",
        )
        return True
...
def check_replay_protection(event_id, timestamp=None):
    ...
    if timestamp:
        ...
        if abs(current_time - float(timestamp)) > window:
            frappe.log_error(
                message=f"Webhook replay detected (stale timestamp): event_id={event_id}",
                title="Mobile Payment Security",
            )
            return False
    ...
    if event_id in _processed_webhooks:
        frappe.log_error(
            message=f"Webhook replay detected (duplicate): event_id={event_id}",
            title="Mobile Payment Security",
        )
        return False
    ...
    if existing:
        frappe.log_error(
            message=f"Webhook replay detected (already completed in DB): event_id={event_id}",
            title="Mobile Payment Security",
        )
        return False
