...
class MobilePaymentSettings(Document):
    ...
    def _validate_edahab_config(self):
        ...
        if self.edahab_enabled:
            required = ["edahab_api_key", "edahab_api_secret", "edahab_number"]
            for field in required:
                if not self.get(field):
                    frappe.throw(
            ...
            edahab_number = "".join(c for c in str(self.edahab_number or "") if c.isdigit())
            if len(edahab_number) != 9 or not edahab_number.startswith(("65", "66", "62", "76")):
                frappe.throw(
                    _("EDahab Number must be 9 digits and start with 65, 66, 62, or 76")
                )
            self.edahab_number = edahab_number
