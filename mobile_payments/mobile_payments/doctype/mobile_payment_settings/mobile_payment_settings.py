...
class MobilePaymentSettings(Document):
    ...
    def _migrate_edahab_base_url(self):
        """
        Auto-correct legacy edahab_base_url values.

        The correct API base is https://edahab.net/api/api — all API calls
        append /IssueInvoice or /CheckInvoiceStatus directly to this.

        Old installs may have https://edahab.net/api (missing the second /api)
        which produces 404 errors. Fix silently on save.
        """
        url = (self.edahab_base_url or "").strip().rstrip("/")
        if url in ("https://edahab.net/api", "http://edahab.net/api"):
            self.edahab_base_url = url.replace("/api", "/api/api", 1)
            frappe.msgprint(
...
def test_edahab_connection():
    ...
    try:
        url = f"{base_url}/api/issueinvoice?hash={request_hash}"
        resp = req.post(
            url,
