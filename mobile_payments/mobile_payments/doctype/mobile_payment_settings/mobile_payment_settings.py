...
class MobilePaymentSettings(Document):
    ...
    def _migrate_edahab_base_url(self):
        """
        Auto-correct any stored edahab_base_url to the required /api/api form.

        The correct API base is always https://edahab.net/api/api — all API
        calls append /IssueInvoice or /CheckInvoiceStatus to this base.

        Handles every common misconfiguration:
          https://edahab.net          → https://edahab.net/api/api
          https://edahab.net/api      → https://edahab.net/api/api
          https://edahab.net/api/api  → unchanged (already correct)
        """
        import re
        url = (self.edahab_base_url or "").strip().rstrip("/")
        if not url:
            return
        # Strip any trailing /api segments (one or two, any case)
        corrected = re.sub(r"(/api)+$", "", url, flags=re.IGNORECASE).rstrip("/") + "/api/api"
        if corrected != url:
            self.edahab_base_url = corrected
            frappe.msgprint(
                _("Edahab Base URL auto-corrected to {0}").format(self.edahab_base_url),
                indicator="orange",
                alert=True,
            )
    ...
    def get_edahab_credentials(self):
        """Return Edahab API credentials with base_url guaranteed in /api/api form."""
        if not self.edahab_enabled:
            frappe.throw(_("Edahab is not enabled"))

        import re
        raw = (self.edahab_base_url or "").strip().rstrip("/")
        normalised = (
            re.sub(r"(/api)+$", "", raw, flags=re.IGNORECASE).rstrip("/") + "/api/api"
            if raw else ""
        )
...
def test_edahab_connection():
    ...
    # Field order matches C# reference: apiKey, edahabNumber, amount, agentCode, currency
    request_data = {
        "apiKey": api_key,
        "edahabNumber": "",
        "amount": 0,
