frappe.pages["mobile-payment-dashboard"].on_page_load = function (wrapper) {
  ...
  /**
   * Format an amount with an explicit currency code prefix,
   * e.g. "USD 1,200.00" or "SLSH 50,000.00".
   * This avoids confusing symbols (both USD & SLSH may show "$").
   */
  function fmt_amount(amount, currency) {
  ...
  /**
   * Format a {currency: amount} dict into a readable string,
   * e.g. "USD 1,200.00 | SLSH 50,000.00".
   * Each currency shown on its own so USD / SLSH are never confused.
   */
  function format_currency_amounts(amount_by_currency) {
    if (!amount_by_currency || !Object.keys(amount_by_currency).length) {
      return fmt_amount(0, "USD");
    }
    return Object.entries(amount_by_currency)
      .map(([currency, amount]) => fmt_amount(amount, currency))
      .join(" &nbsp;|&nbsp; ");
  }
    ...
    let stats_html = `
      <div class="col-sm-6 col-md-3">
        <div class="stat-card success" style="cursor:pointer;" data-filter="Completed">
          ...
          <small class="text-muted">${format_currency_amounts(summary.amount_by_currency)}</small>
        </div>
      </div>
      <div class="col-sm-6 col-md-3">
  ...
  function render_provider_breakdown(providers) {
    ...
    let rows = providers
      ...
      <tr>
        ...
        <td style="text-align:right;">${fmt_amount(p.total_amount, p.currency)}</td>
      </tr>
    `
      )
      .join("");
        ...
        if (r.message && r.message.length) {
          let rows = r.message
            ...
            <tr>
              ...
              <td style="text-align:right;">${fmt_amount(t.amount, t.currency)}</td>
