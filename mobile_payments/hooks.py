...
# DocType-specific JS (loaded only when this doctype form opens)
doctype_js = {
...
website_route_rules = [
    ...
    {
        "from_route": "/payment-failed",
        "to_route": "payment_failed",
    },
]
...
doc_events = {
    "Sales Invoice": {
        ...
        "on_cancel": [
            "mobile_payments.utils.payment_links.invalidate_payment_links_on_cancel",
        ],
    },
    "Sales Order": {
    ...
    "Payment Entry": {
        "after_insert": "mobile_payments.utils.payment_links.generate_pe_payment_links",
...
scheduler_events = {
    "cron": {
        ...
        "*/2 * * * *": [
            ...
            "mobile_payments.utils.payment_handler.retry_pending_payment_entries",
        ],
        # HPP sessions are refreshed on-visit only (no background polling needed)
        # Edahab uses self-hosted page (Purchase API), WaafiPay refreshes on page load
        # Daily reconciliation at 1 AM
        "0 1 * * *": [
            "mobile_payments.utils.reconciliation.run_daily_reconciliation",
        ],
        # Generate payment links for invoices due today (runs every hour)
        "0 * * * *": [
...
override_whitelisted_methods = {
    "posawesome.posawesome.api.posapp.create_payment_request": (
        "mobile_payments.api.pos.create_payment_request_override"
    ),
}
...
after_migrate = "mobile_payments.install.after_migrate"
