...
def _create_custom_fields():
    """Add custom fields to Sales Invoice, POS Invoice, and Payment Entry."""
    from frappe.custom.doctype.custom_field.custom_field import create_custom_fields

    # Determine a safe insert_after for Sales Invoice.
    # Different ERPNext versions use different section names.
    si_insert_after = "payments_section"
    si_meta = frappe.get_meta("Sales Invoice")
    if not si_meta.has_field(si_insert_after):
        # Try common alternatives
        for candidate in ["payments_tab", "payments", "base_grand_total",
                          "grand_total", "terms_section_break", "more_info"]:
            if si_meta.has_field(candidate):
                si_insert_after = candidate
                break

    # Same for POS Invoice
    pos_insert_after = "payments_section"
    if frappe.db.exists("DocType", "POS Invoice"):
        pos_meta = frappe.get_meta("POS Invoice")
        if not pos_meta.has_field(pos_insert_after):
            for candidate in ["payments_tab", "payments", "base_grand_total",
                              "grand_total", "terms_section_break", "more_info"]:
                if pos_meta.has_field(candidate):
                    pos_insert_after = candidate
                    break

    # -- Mobile Payment fields shared by Sales Invoice and POS Invoice --
    def _mobile_payment_fields(insert_after):
        return [
            {
    ...
    custom_fields = {
        "Sales Invoice": _mobile_payment_fields(si_insert_after) + [
            {
    ...
    # Add fields to POS Invoice if it exists
    if frappe.db.exists("DocType", "POS Invoice"):
        custom_fields["POS Invoice"] = _mobile_payment_fields(pos_insert_after)
...
def _remove_custom_fields():
    ...
    fields_to_remove = {
        "Sales Invoice": _mobile_field_names + [
            "payment_links_section",
            "waafi_payment_link",
            "payment_links_column_break",
            "edahab_payment_link",
        ],
        "POS Invoice": _mobile_field_names,
        "Patient Appointment": _mobile_field_names + [
