...
def get_draft_pos_invoice():
    """
    Return the latest draft POS Invoice for the current user.
    Used as a fallback when the JS frontend cannot extract the invoice
    doc from POS Awesome's Vue 3 store.

    Returns:
        dict: {name, doctype} or None
    """
    user = frappe.session.user

    # Try POS Invoice first (POS Awesome creates these)
    ...
    if name:
        return {"name": name, "doctype": "POS Invoice"}

    # Fallback: regular Sales Invoice marked as POS
    name = frappe.db.get_value(
        "Sales Invoice",
        filters={"docstatus": 0, "is_pos": 1, "owner": user},
        fieldname="name",
        order_by="modified desc",
    )
    if name:
        return {"name": name, "doctype": "Sales Invoice"}
