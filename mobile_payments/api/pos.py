...
def link_pos_invoice(transaction_log, invoice_name):
    ...
    Also populates the Mobile Payment fields on the invoice.
    ...
    # Populate Mobile Payment fields on the invoice
    for doctype in ("POS Invoice", "Sales Invoice"):
        if frappe.db.exists(doctype, invoice_name):
            update_fields = {}
            if frappe.get_meta(doctype).has_field("mobile_payment_status"):
                update_fields["mobile_payment_status"] = log.status or "Completed"
            if frappe.get_meta(doctype).has_field("mobile_payment_provider"):
                update_fields["mobile_payment_provider"] = log.provider or ""
            if frappe.get_meta(doctype).has_field("mobile_payment_method"):
                update_fields["mobile_payment_method"] = log.payment_method or ""
            if frappe.get_meta(doctype).has_field("mobile_payment_phone"):
                update_fields["mobile_payment_phone"] = log.phone_number or ""
            if frappe.get_meta(doctype).has_field("mobile_payment_reference"):
                update_fields["mobile_payment_reference"] = (
                    log.provider_transaction_id or log.provider_reference or ""
                )
            if frappe.get_meta(doctype).has_field("mobile_payment_transaction_log"):
                update_fields["mobile_payment_transaction_log"] = log.name or ""

            if update_fields:
                frappe.db.set_value(doctype, invoice_name, update_fields,
                                    update_modified=False)
                frappe.logger("mobile_payments").info(
                    f"POS mobile payment fields updated on {doctype} {invoice_name}: "
...
def check_pos_payment_status(transaction_log):
    ...
    When a pending payment completes, the Payment Entry / Sales Invoice is
    created **inline** (synchronously) so the frontend gets the PE and SI
    references in the same response.  A background-only ``frappe.enqueue``
    previously caused PE creation to be silently skipped when workers were
    slow or the queue was backed up — the frontend already showed "success"
    but the invoice never became "Paid".

    Args:
        transaction_log: Transaction Log name

    Returns:
        dict with status details including payment_entry and sales_invoice
    """
    log = frappe.get_doc("Mobile Payment Transaction Log", transaction_log)
    ...
    result = {
        ...
        "payment_entry": log.payment_entry or "",
        "sales_invoice": log.sales_invoice or "",
    ...
    if log.status in ("Pending", "Initiated"):
        try:
            ...
            if log.provider == "Edahab":
                from mobile_payments.api.edahab import EdahabClient
                client = EdahabClient()
                # Edahab needs the InvoiceId (stored in provider_reference), not transaction_id
            ...
            # ── Payment confirmed — create PE/SI inline ─────────────
            if completed:
                frappe.db.set_value("Mobile Payment Transaction Log", log.name, {
                    "status": "Completed",
                ...
                # Run payment processing INLINE (not enqueued) so the
                # PE/SI are created within this request and returned to
                # the frontend immediately.  Fall back to enqueue only
                # if inline processing raises an unexpected error.
                _process_completed_payment_inline(log, result)
    ...
    # If status is already Completed but PE is missing, try inline again
    # (covers edge case where a prior poll marked Completed but PE
    # creation failed or was never triggered).
    elif log.status == "Completed" and not log.payment_entry:
        _process_completed_payment_inline(log, result)

    return result


def _process_completed_payment_inline(log, result):
    """
    Create Payment Entry / Sales Invoice synchronously within the current
    web request so the frontend gets the references immediately.

    Handles three source types:
      1. Patient Appointment (no SI yet) → create SI + PE via appointment handler
      2. Sales Invoice already linked     → create PE via standard handler
      3. POS (no SI, no appointment)      → enqueue for later (POS links invoice on submit)

    Args:
        log: Mobile Payment Transaction Log document (already marked Completed)
        result: mutable dict — PE / SI references are added in-place
    """
    try:
        if log.patient_appointment and not log.sales_invoice and not log.payment_entry:
            from mobile_payments.utils.payment_handler import (
                process_successful_appointment_payment,
            )
            process_successful_appointment_payment(log.name)

            # Reload log to pick up newly created SI + PE
            log.reload()
            result["sales_invoice"] = log.sales_invoice or ""
            result["payment_entry"] = log.payment_entry or ""

        elif log.sales_invoice and not log.payment_entry:
            from mobile_payments.utils.payment_handler import (
            ...
            # Reload log to pick up newly created PE
            log.reload()
            result["payment_entry"] = log.payment_entry or ""
            result["sales_invoice"] = log.sales_invoice or ""

    except Exception as e:
        # Inline processing failed — fall back to background enqueue
        # so the scheduler / retry logic can pick it up later.
        frappe.log_error(
            message=(
                f"Inline payment processing failed for {log.name}, "
                f"falling back to enqueue: {str(e)}\n{frappe.get_traceback()}"
            ),
            title="Inline Payment Processing Error",
        )

        if log.patient_appointment and not log.sales_invoice and not log.payment_entry:
            frappe.enqueue(
                "mobile_payments.utils.payment_handler.process_successful_appointment_payment",
                transaction_log=log.name,
                queue="short",
            )
        elif log.sales_invoice and not log.payment_entry:
...
def get_customer_phone(customer):
    ...
    Lookup order:
    0. Resolve customer name to ID if needed
    ...
    Args:
        customer: Customer name/ID or customer_name
    ...
    # Resolve: if the value isn't a valid Customer ID, try matching by customer_name
    if not frappe.db.exists("Customer", customer):
        match = frappe.db.get_value("Customer", {"customer_name": customer}, "name")
        if match:
            customer = match
        else:
            return {"phone": "", "source": ""}
...
# ── Edahab prefix list (Somaliland Telesom network) ──
EDAHAB_PREFIXES = ("65", "66", "62", "76")


def _normalize_phone(phone):
    """Strip spaces, dashes, plus, and country code to get local digits."""
    if not phone:
        return ""
    p = phone.strip().replace(" ", "").replace("-", "").replace("(", "").replace(")", "")
    if p.startswith("+"):
        p = p[1:]
    # Strip country code 252
    if p.startswith("252") and len(p) > 9:
        p = p[3:]
    # Strip leading 0
    if p.startswith("0") and len(p) > 7:
        p = p[1:]
    return p


def _is_edahab_number(phone):
    """Check if a phone number belongs to an Edahab-compatible network."""
    local = _normalize_phone(phone)
    return any(local.startswith(prefix) for prefix in EDAHAB_PREFIXES)


def _collect_all_phones(customer):
    """
    Collect ALL phone numbers for a customer from every source.
    Returns a list of dicts: [{phone: "...", source: "..."}, ...]
    """
    phones = []
    seen = set()
    ...
    def _add(p, src):
        ...
        if clean and clean not in seen:
            seen.add(clean)
            phones.append({"phone": p.strip(), "source": src})
    ...
    try:
        ...
        for contact in contacts:
            _add(contact.mobile_no, "Contact")
            _add(contact.phone, "Contact")
            ...
            for entry in phone_entries:
                _add(entry.phone, "Contact Phone")

        # 2. Customer.mobile_no
        cust = frappe.db.get_value("Customer", customer, ["mobile_no"], as_dict=True)
        if cust and cust.mobile_no:
            _add(cust.mobile_no, "Customer")

        # 3. Patient linked to Customer (Healthcare)
        try:
            patient = frappe.db.get_value(
                "Patient", {"customer": customer}, ["mobile", "phone"], as_dict=True
            )
            if patient:
                _add(patient.mobile, "Patient")
                _add(patient.phone, "Patient")
        except Exception:
            pass

    except Exception as e:
        frappe.log_error(
            message=f"Phone collect error for {customer}: {str(e)}",
            title="Phone Collect Error",
        )
...
@frappe.whitelist()
def get_customer_phone_for_provider(customer, provider=None):
    """
    Smart phone routing: pick the right phone number based on the provider.

    Edahab numbers: prefixes 65, 66, 62, 76 (Telesom network)
    WaafiPay numbers: everything else (Hormuud/Golis/Somtel)

    If provider is specified, returns the best match for that provider.
    If not specified, returns all phones with provider hints.

    Args:
        customer: Customer name/ID
        provider: "Edahab" or "WaafiPay" (optional)

    Returns:
        dict: {phone, source, all_phones: [{phone, source, provider_hint}]}
    """
    all_phones = _collect_all_phones(customer)

    # Tag each phone with a provider hint
    for p in all_phones:
        p["provider_hint"] = "Edahab" if _is_edahab_number(p["phone"]) else "WaafiPay"

    edahab_phones = [p for p in all_phones if p["provider_hint"] == "Edahab"]
    waafi_phones = [p for p in all_phones if p["provider_hint"] == "WaafiPay"]

    best_phone = ""
    best_source = ""

    if provider and provider.lower() == "edahab":
        if edahab_phones:
            best_phone = edahab_phones[0]["phone"]
            best_source = edahab_phones[0]["source"]
        # Fallback: use first available (better than nothing)
        elif all_phones:
            best_phone = all_phones[0]["phone"]
            best_source = all_phones[0]["source"]
    elif provider and provider.lower() in ("waafipay", "waafi"):
        if waafi_phones:
            best_phone = waafi_phones[0]["phone"]
            best_source = waafi_phones[0]["source"]
        elif all_phones:
            best_phone = all_phones[0]["phone"]
            best_source = all_phones[0]["source"]
    else:
        # No provider specified — return first available
        if all_phones:
            best_phone = all_phones[0]["phone"]
            best_source = all_phones[0]["source"]
    ...
    return {
        "phone": best_phone,
        "source": best_source,
        "edahab_phone": edahab_phones[0]["phone"] if edahab_phones else "",
        "waafi_phone": waafi_phones[0]["phone"] if waafi_phones else "",
...
SUCCESS_STATES = {"SUCCESS", "PAID", "COMMITTED", "CAPTURED", "SETTLED"}

@frappe.whitelist()
def commit_preauthorize(invoice_name: str, provider_txn_id: str | None = None):
    """
    Commit a preauthorized transaction for a POS invoice.

    Args:
        invoice_name: Sales Invoice or POS Invoice name
        provider_txn_id: Provider transaction ID (optional)

    Returns:
        dict: {ok: True, idempotent: True, status: "COMMITTED", result: commit_result}
    """
    txn = _get_mobile_payment_txn(invoice_name, provider_txn_id)  # ...existing helper...
    txn_type = (txn.get("authorization_type") or txn.get("txn_type") or "").upper()
    status = (txn.get("provider_status") or txn.get("status") or "").upper()

    # Not a preauth flow -> skip commit
    if txn_type not in {"PREAUTH", "PREAUTHORIZE"}:
        return {"ok": True, "skipped": True, "reason": "not_preauth"}

    # Already finalized -> idempotent success
    if status in {"SUCCESS", "PAID", "COMMITTED", "CAPTURED", "SETTLED"}:
        return {"ok": True, "idempotent": True, "status": status}

    try:
        result = _provider_commit(txn)  # ...existing provider commit...
        _mark_txn_committed(txn, result)  # ...existing status update...
        return {"ok": True, "status": "COMMITTED"}
    except Exception as e:
        msg = str(e).lower()
        if "not allowed for the current transaction status" in msg:
            return {"ok": True, "idempotent": True, "status": "ALREADY_FINALIZED"}
        raise


@frappe.whitelist()
def get_current_draft_pos_invoice(pos_profile=None, customer=None):
    """
    Find the most recent draft POS Invoice for the current user.
    Used as a fallback when the JS cannot extract the invoice from the Vue store.
    """
    filters = {
        "docstatus": 0,
    ...
    if not name and pos_profile:
        # Try without pos_profile filter
    ...
    if not name:
        # Try without owner filter (POS Awesome may save under admin)
        fallback_filters = {"docstatus": 0}
        if customer:
            fallback_filters["customer"] = customer
        if pos_profile:
            fallback_filters["pos_profile"] = pos_profile
        name = frappe.db.get_value(
            "POS Invoice",
...
@frappe.whitelist()
def link_invoice_to_transaction(transaction_log, invoice_name):
    """
    Called after POS submit — links the final Sales Invoice name to the
    transaction log and updates all mobile payment fields on the invoice.
    """
    try:
        log = frappe.get_doc("Mobile Payment Transaction Log", transaction_log)

        # Update the sales_invoice field on the log
        if log.sales_invoice != invoice_name:
            log.sales_invoice = invoice_name
            log.save(ignore_permissions=True)

        # Update mobile payment fields on the Sales Invoice
        ...
        frappe.logger("mobile_payments").info(
            f"link_invoice_to_transaction: updated {doctype} {invoice_name} "
            f"from log {log.name}"
        )
        ...
        frappe.log_error(
            message=f"link_invoice_to_transaction failed: {str(e)}",
            title="Mobile Payment Link Error",
        )
        return {"success": False, "message": str(e)}


@frappe.whitelist()
def link_latest_pos_invoice(transaction_log):
    """
    Find the most recently submitted Sales Invoice (from POS) and link it
    to the transaction log. Used when the JS couldn't find the invoice doc
    directly and had to click POS Awesome's submit button.

    Looks for a Sales Invoice submitted in the last 60 seconds that:
    - is a POS invoice (is_pos=1)
    - is not already linked to another transaction
    """
    try:
        log = frappe.get_doc("Mobile Payment Transaction Log", transaction_log)
        ...
        if log.sales_invoice:
            inv = log.sales_invoice
            # Check if fields are already set
            status = frappe.db.get_value("Sales Invoice", inv, "mobile_payment_status")
            if status == "Completed":
                return {"success": True, "invoice_name": inv, "already_linked": True}
            # Fields not set — re-link
            link_pos_invoice(transaction_log, inv)
            return {"success": True, "invoice_name": inv}

        # Find recent POS invoice — submitted in the last 60 seconds
        ...
        if not recent:
            # Also check POS Invoice doctype
            recent = frappe.get_all(
                "POS Invoice",
                filters={
                    "docstatus": 1,
        ...
        if not recent:
            return {"success": False, "message": "No recent invoice found yet"}

        invoice_name = recent[0].name
        result = link_pos_invoice(transaction_log, invoice_name)
        result["invoice_name"] = invoice_name
        return result

    except Exception as e:
        frappe.log_error(
            message=f"link_latest_pos_invoice failed: {str(e)}",
            title="Mobile Payment Link Error",
        )
        return {"success": False, "message": str(e)}
