...
def commit_preauthorize(invoice_name: str, provider_txn_id: str | None = None):
    ...
    """
    txn = _get_mobile_txn(invoice_name, provider_txn_id)  # use your existing helper/query

    current = (txn.get("provider_status") or txn.get("status") or "").upper()
    if current in SUCCESS_STATES:
        return {"ok": True, "idempotent": True, "status": current, "message": "Already committed"}

    try:
        # ...existing provider commit call...
        commit_result = _provider_commit(txn)  # your existing call
        # ...existing status update...
        return {"ok": True, "status": "COMMITTED", "result": commit_result}
    except Exception as e:
        msg = str(e).lower()
        if "not allowed for the current transaction status" in msg:
            # Convert provider conflict into idempotent success
            return {
                "ok": True,
                "idempotent": True,
