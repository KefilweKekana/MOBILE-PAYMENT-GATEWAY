...
def initiate_pos_payment(provider, method, phone, amount, currency=None,
    ...
    try:
        ...
        if result.get("success"):
            ...
            # For Edahab, build the HPP URL so the frontend can open it
            hpp_url = None
            if provider == "Edahab":
                edahab_invoice_id = result.get("invoice_id", "")
                if edahab_invoice_id:
                    from mobile_payments.api.edahab import EdahabClient
                    edahab_client = EdahabClient()
                    hpp_base = (edahab_client.hpp_base_url or "https://edahab.net").rstrip("/")
                    hpp_url = f"{hpp_base}/API/Payment?invoiceId={edahab_invoice_id}"
