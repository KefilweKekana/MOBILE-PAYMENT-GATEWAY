...
def initiate_pos_payment(provider, method, phone, amount, currency=None,
    ...
    # Validate phone number - BLOCK if missing or invalid
    ...
    if not phone:
        return {
            "success": False,
            "message": _(
                "Customer mobile number is required for mobile payment. "
                "Please add a mobile number to the customer record."
            ),
        }
    if not phone.isdigit():
        return {
            "success": False,
            "message": _("Invalid phone number. Must contain only digits (e.g. 252612345678)."),
        }
    ...
    # Build payment description WITH invoice number (required by spec)
