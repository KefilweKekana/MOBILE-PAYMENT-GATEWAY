...
class EdahabClient:
    ...
    def purchase_request(self, phone, amount, invoice_id=None,
        ...
        try:
            ...
            frappe.log_error(error_msg, "Edahab API Error")
            ...
            frappe.log_error(frappe.get_traceback(), "Edahab Error")
            log.update_status("Failed", error_message=error_msg)
            return {
                "success": False,
                "message": error_msg,
    ...
    def create_hpp_session(self, amount, invoice_id=None, description="",
        ...
        try:
            ...
            api_url = f"{self.hpp_base_url}/api/issueinvoice"
            response = requests.post(
                api_url,
                json=payload,
                headers=self._get_headers(),
            ...
            frappe.log_error(frappe.get_traceback(), "Edahab HPP Error")
            log.update_status("Failed", error_message=error_msg)
            return {
                "success": False,
                "message": error_msg,
...
@frappe.whitelist()
def initiate_edahab_hpp(amount, invoice_id=None, description=""):
    """
