...
class EdahabClient:
    ...
    def create_hpp_session(self, amount, invoice_id=None, description="",
        ...
        try:
            ...
            if int(status_code) == 0:
                ...
                # still contains the existing InvoiceId.
                #
                # Before redirecting, verify the invoice is still valid on
                # Edahab's portal.  Expired/invalid invoices show an
                # "Invalid Invoice" page — terrible UX.
                frappe.logger("mobile_payments").info(
                    f"Edahab HPP StatusCode 7 — checking existing invoice "
                    f"{edahab_invoice_id} status before redirect"
                )

                inv_check = self.check_transaction_status(edahab_invoice_id)
                inv_status = (inv_check.get("invoice_status") or "").strip().lower()
                frappe.logger("mobile_payments").info(
                    f"Edahab invoice {edahab_invoice_id} status: "
                    f"{inv_status} | raw: {inv_check}"
                )

                if inv_status in ("pending", "unpaid", ""):
                    # Invoice is still valid — redirect to it
                    hpp_url = (
                        f"{self.hpp_base_url}/API/Payment?invoiceId={edahab_invoice_id}"
                    )
                    log.update_status("Pending", response_payload=data)
                    ...
                    # Invoice expired/paid/cancelled on Edahab's side but
                    # their API still blocks new creation (409).  The customer
                    # must wait for Edahab to clear the stale invoice.
                    error_msg = _(
                        "Edahab has a previous payment session that has expired "
                        "but not yet been cleared.  Please wait 2–3 minutes and "
                        "try again, or use USSD Push instead."
                    )
                    frappe.logger("mobile_payments").warning(
                        f"Edahab HPP StatusCode 7 — existing invoice "
                        f"{edahab_invoice_id} is stale (status={inv_status}). "
                        f"Cannot redirect or create new."
                    )
                    log.update_status("Failed", error_message=error_msg,
                                      response_payload=data)
                    return {
    ...
    def _get_or_create_log(self, transaction_log=None, phone="", amount=0,
        ...
        if transaction_log:
            ...
            if invoice_id and not doc.sales_invoice:
