...
class EdahabClient:
    ...
    def create_hpp_session(self, amount, invoice_id=None, description="",
        ...
        try:
            api_url = f"{self.base_url}/api/issueinvoice?hash={request_hash}"  # Same endpoint as purchase; hpp_base_url is only for the customer redirect
            frappe.logger("mobile_payments").info(
...
def debug_edahab_hash():
    ...
    return {
        ...
        "purchase": {
            "body": purchase_body,
            "hash": purchase_hash,
            "url": f"{base_url}/api/issueinvoice?hash={purchase_hash}",
        },
        "hpp": {
            "body": hpp_body,
            "hash": hpp_hash,
            "url": f"{hpp_base_url}/api/api/issueinvoice?hash={hpp_hash}",
        },
    }
