...
class EdahabClient:
    ...
    def _generate_hash(self, payload_dict):
        ...
        Per the official Edahab SDK, hash = SHA256(JSON(payload) + secret).

        The payload is serialised with compact separators (',', ':') to match
        JavaScript's JSON.stringify() which the Edahab server expects.

        Args:
            payload_dict: The request payload dict (will be JSON-serialised)

        Returns:
            tuple(hash_hex, json_body_string)
        """
    ...
    def purchase_request(self, phone, amount, invoice_id=None,
        ...
        # Edahab SDK field names (camelCase) – must match for hash
        payload = {
            "apiKey": self.api_key,
            "edahabNumber": phone,
            "amount": amount_val,
        ...
        request_hash, body_string = self._generate_hash(payload)
        ...
        try:
            ...
            api_url = f"{self.base_url}/api/issueinvoice?hash={request_hash}"
    ...
    def create_hpp_session(self, amount, invoice_id=None, description="",
        ...
        try:
            ...
            api_url = f"{self.hpp_base_url}/api/api/issueinvoice?hash={request_hash}"
            response = requests.post(
                api_url,
                data=body_string,
                headers=self._get_headers(),
    ...
    def check_transaction_status(self, invoice_id):
        ...
        try:
            api_url = f"{self.base_url}/api/CheckInvoiceStatus?hash={request_hash}"
            response = requests.post(
                api_url,
                data=body_string,
                headers=self._get_headers(),
