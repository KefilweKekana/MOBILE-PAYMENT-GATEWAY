...
class EdahabClient:
    ...
    def __init__(self):
        ...
        # Normalise base_url regardless of how it was entered in settings.
        # Edahab's API endpoints live under /api/api/ on their domain.
        # All of these inputs map to https://edahab.net/api/api:
        #   https://edahab.net
        #   https://edahab.net/api
        #   https://edahab.net/api/api   (already correct)
        import re as _re
        _raw = (self.credentials["base_url"] or "").strip().rstrip("/")
        if _raw:
            _base = _re.sub(r"(/api)+$", "", _raw, flags=_re.IGNORECASE).rstrip("/")
            self.base_url = _base + "/api/api"
        else:
            self.base_url = ""
