...
class EdahabClient:
    ...
    def _process_purchase_response(self, data, log, reference):
        ...
        elif int(status_code) == 7:
            # StatusCode 7 = an active invoice already exists for this
            # agentCode + edahabNumber.  Edahab refused to create a new
            # one and NO new USSD prompt was sent.  Unlike HPP (where we
            # can reuse the InvoiceId to redirect), USSD has nothing to
            # reuse — just tell the user to wait and retry.
            frappe.logger("mobile_payments").info(
                f"Edahab Purchase StatusCode 7 — active invoice "
                f"{invoice_id} blocks new USSD for this number"
            )
            error_msg = (
                "This Edahab number already has a pending payment. "
                "Please wait a few minutes for it to expire, then try again."
            )
            log.update_status("Failed", error_message=error_msg,
                              error_code="7", response_payload=data)
            return {
                "success": False,
                "message": error_msg,
                "status_code": 7,
