    ...
    try:
        ...
        if log:
            if is_success:
                ...
                from mobile_payments.utils.payment_handler import process_successful_payment
                # Use now=True in the HPP return path so the invoice is marked paid
                # before we redirect the customer — avoids "still pending" confusion.
                frappe.enqueue(
                    process_successful_payment,
                    queue="short",
                    transaction_log=log.name,
    ...
    try:
        ...
        if log:
            if is_success:
                ...
                from mobile_payments.utils.payment_handler import process_successful_payment
                # Run synchronously so the invoice is paid before customer is redirected
                frappe.enqueue(
                    process_successful_payment,
                    queue="short",
                    transaction_log=log.name,
