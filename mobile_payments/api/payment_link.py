...
def pay(token):
    ...
    # ── Try to reuse an existing active HPP session for this invoice ──
    # This avoids creating duplicate Edahab IssueInvoice sessions that
    # tie up the agent balance and cause "insufficient balance" errors.
    # Only reuse sessions created within the last 30 minutes — provider HPP
    # tokens expire (Edahab ~1 hr, WaafiPay ~15–30 min), so older sessions
    # would redirect the customer to a dead page.
    provider = log.provider
    from frappe.utils import add_to_date
    hpp_reuse_cutoff = add_to_date(now_datetime(), minutes=-30)
    ...
    if existing_hpp and existing_hpp.hpp_url:
        ...
        frappe.logger("mobile_payments").info(
            f"Reusing recent HPP session for {invoice_id}: {hpp_url[:60]}..."
        )
        # Track visit
