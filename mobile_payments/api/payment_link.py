...
def edahab_initiate_payment(token, phone):
    ...
    try:
        ...
        # If Edahab returned instant success, enqueue PE creation.
        # MUST use enqueue (background worker) — inline PE creation fails
        # in the guest web-request context despite permission escalation.
        # Background workers run as Administrator in a separate process.
        if result.get("success") and invoice_id:
            ...
            frappe.logger("mobile_payments").info(
                f"Enqueued PE creation for Edahab instant success | log={log.name} | invoice={invoice_id}"
            )
    ...
    if log.status in ("Pending", "Initiated", "Failed") and log.provider_reference:
        try:
            ...
            if invoice_status == "Paid":
                ...
                # Enqueue PE creation in background worker
                if log.sales_invoice and not log.payment_entry:
                    ...
                    frappe.logger("mobile_payments").info(
                        f"Enqueued PE creation for Edahab poll success | log={log.name}"
                    )
