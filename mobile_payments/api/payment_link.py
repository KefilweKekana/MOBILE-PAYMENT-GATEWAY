...
def edahab_initiate_payment(token, phone):
    ...
    try:
        ...
        # If Edahab returned instant success, create PE right away
        # (purchase_request sets log status = Completed but doesn't create PE
        #  when called from the payment link guest context)
        if result.get("success") and invoice_id:
            try:
                ...
                frappe.log_error(
                    message=(
                        f"Inline PE failed for Edahab payment link instant success: {pe_err}\n"
                        f"{frappe.get_traceback()}"
                    ),
                    title="Edahab Payment Link PE Error",
                )
                # Fallback: enqueue runs as Administrator in background worker
                try:
                    frappe.enqueue(
            ...
            if invoice_status == "Paid":
                ...
                if log.sales_invoice and not log.payment_entry:
                    ...
                    try:
                        ...
                        frappe.log_error(
                            message=f"Inline PE creation failed for payment link status check, falling back to enqueue: {e}",
                            title="Payment Link Inline PE Error",
                        )
                        frappe.enqueue(
                            "mobile_payments.utils.payment_handler.process_successful_payment",
                            transaction_log=log.name,
