...
from frappe.utils import now_datetime, get_datetime, time_diff_in_seconds, cint, flt
...
def _get_charge_amount(invoice_id):
    ...
    For a fully-unpaid invoice this returns ``grand_total``.
    For a partly-paid invoice it returns only the outstanding portion,
    converted back into the transaction currency when they differ.

    ERPNext stores ``outstanding_amount`` in the **company** currency.
    ``grand_total`` / ``base_grand_total`` let us convert proportionally:
        outstanding_txn = grand_total × (outstanding_amount / base_grand_total)
    """
    ...
    base_grand = flt(inv.base_grand_total) or grand
    outstanding = flt(inv.outstanding_amount)

    # Fully unpaid — return full grand_total (avoids rounding drift)
    if abs(outstanding - base_grand) < 0.01:
        return grand

    # Partly paid — compute outstanding in the transaction currency
    if base_grand > 0:
        return round(grand * (outstanding / base_grand), 2)

    # Fallback via conversion_rate
    rate = flt(inv.conversion_rate) or 1
...
def create_payment_link(invoice_id, provider=None, method=None, expiry_hours=24,
    ...
    # Charge only the outstanding amount (in the invoice's own currency)
...
def pay(token):
    ...
    if log.status == "Completed":
        frappe.respond_as_web_page(
            _("Already Paid"),
            _("This payment has already been completed. Thank you!"),
            http_status_code=200,
            indicator_color="green",
        )
        return

    # ── Determine source doctype (Sales Invoice or Payment Entry) ──
    invoice_id = log.sales_invoice
    pe_id = log.payment_entry if hasattr(log, "payment_entry") else None
    source_doctype = None
    source_name = None

    if invoice_id and frappe.db.exists("Sales Invoice", invoice_id):
        source_doctype = "Sales Invoice"
        source_name = invoice_id
    elif pe_id and frappe.db.exists("Payment Entry", pe_id):
        source_doctype = "Payment Entry"
        source_name = pe_id
    else:
        frappe.respond_as_web_page(
            _("Document Not Found"),
            _("The associated payment document could not be found."),
            http_status_code=404,
            indicator_color="red",
        )
        return

    # Block if the source document has been cancelled (docstatus == 2)
    src_docstatus = frappe.db.get_value(source_doctype, source_name, "docstatus")
    if src_docstatus == 2:
        if log.payment_link_token:
            log.db_set("payment_link_token", None)
            if log.status not in ("Cancelled", "Failed"):
                log.db_set("status", "Cancelled")
            frappe.db.commit()
        frappe.respond_as_web_page(
            _("Document Cancelled"),
            _("This document has been cancelled. The payment link is no longer valid."),
            http_status_code=410,
            indicator_color="red",
        )
        return

    # ── Determine amount and currency based on source doctype ──
    if source_doctype == "Sales Invoice":
        ...
        invoice_is_paid = (
            ...
            or frappe.db.exists(
                ...
                {"reference_name": source_name, "docstatus": 1},
            )
        )
        ...
        charge_amount = _get_charge_amount(source_name) or float(inv_data.grand_total or 0)
        charge_currency = inv_data.currency or "USD"
    else:
        # Payment Entry
        pe_data = frappe.db.get_value(
            "Payment Entry",
            source_name,
            ["paid_amount", "received_amount", "paid_to_account_currency",
             "paid_from_account_currency", "docstatus"],
            as_dict=True,
        )
        if not pe_data:
            frappe.respond_as_web_page(
                _("Document Not Found"),
                _("The associated payment entry could not be found."),
                http_status_code=404,
                indicator_color="red",
            )
            return
        ...
        pe_mobile_status = ""
        if frappe.get_meta("Payment Entry").has_field("mobile_payment_status"):
            pe_mobile_status = frappe.db.get_value("Payment Entry", source_name, "mobile_payment_status") or ""

        # Also check if ANY other log for this PE already completed
        # (e.g. payment made via WaafiPay, now visiting the Edahab link)
        any_completed_log = frappe.db.exists(
            "Mobile Payment Transaction Log",
            {"payment_entry": source_name, "status": "Completed"},
        )

        if pe_data.docstatus == 1 or pe_mobile_status == "Completed" or any_completed_log:
            if log.status != "Completed":
                log.db_set("status", "Completed")
            log.db_set("payment_link_token", None)
            frappe.db.commit()
        ...
        charge_amount = flt(pe_data.paid_amount) or flt(pe_data.received_amount)
        charge_currency = pe_data.paid_to_account_currency or pe_data.paid_from_account_currency or "USD"
    ...
    if source_doctype == "Sales Invoice":
        hpp_filter["sales_invoice"] = source_name
    else:
        hpp_filter["payment_entry"] = source_name
    ...
    # Push to their phone.
    if provider == "Edahab":
        # Always do a fresh provider-specific phone lookup.
        # log.phone_number may contain a WaafiPay number stored at
        # link-generation time when no provider was specified.
        prefill_phone = ""
        customer = None
        if source_doctype == "Sales Invoice":
            customer = frappe.db.get_value("Sales Invoice", source_name, "customer")
        else:
            customer = frappe.db.get_value("Payment Entry", source_name, "party")
        if customer:
            try:
                from mobile_payments.api.pos import get_customer_phone_for_provider
        ...
        frappe.logger("mobile_payments").info(
            f"Edahab payment link visited: {token[:8]}... | {source_doctype}: {source_name} | "
            f"Visit #{visit_count} | Serving self-hosted payment page"
        )

        _render_edahab_payment_page(
            token, source_name, charge_amount,
            charge_currency, prefill_phone,
        )
        return

    # ── WaafiPay: create HPP session and redirect ──
    amount = charge_amount
    description = f"Payment for {source_name}"
    # Only pass invoice_id for Sales Invoices — PE names would fail
    # Link validation on the sales_invoice field in _get_or_create_log.
    si_id = source_name if source_doctype == "Sales Invoice" else None
    ...
    try:
        ...
        result = client.create_hpp_session(
            ...
            currency=log.currency or charge_currency or "USD",
            transaction_log=log.name,
        )
        ...
        if result.get("success") and result.get("hpp_url"):
            ...
            frappe.logger("mobile_payments").info(
                f"Payment link visited: {token[:8]}... | {source_doctype}: {source_name} | "
...
def edahab_initiate_payment(token, phone):
    ...
    # Already paid
    if log.status == "Completed":
        return {"success": True, "message": _("This payment has already been completed.")}
    ...
    if invoice_id and frappe.db.exists("Sales Invoice", invoice_id):
        ...
        return {"success": False, "message": _("Payment document not found.")}
    ...
    if source_doctype == "Sales Invoice":
        ...
        pe_data = frappe.db.get_value(
            "Payment Entry", source_name,
            ["paid_amount", "received_amount", "paid_to_account_currency",
             "paid_from_account_currency", "docstatus"],
            as_dict=True,
        )
        if not pe_data:
            return {"success": False, "message": _("Payment entry not found.")}
    ...
    try:
        ...
        result = client.purchase_request(
            ...
            invoice_id=si_id,
            description=f"Payment for {source_name}",
            currency=log.currency or charge_currency,
            transaction_log=log.name,
        )

        frappe.logger("mobile_payments").info(
            f"Edahab USSD initiated from payment link {token[:8]}... | "
            f"{source_doctype}: {source_name} | Phone: {phone} | Result: {result.get('pending', False)}"
        )
        ...
        if result.get("success") and source_name:
            frappe.db.commit()
            ...
            try:
                process_successful_payment(
                    transaction_log=log.name,
                    invoice_id=source_name if source_doctype == "Sales Invoice" else None,
                )
            except Exception as pe_err:
                frappe.log_error(
                    message=(
                        f"Inline PE failed for Edahab payment link: {pe_err}\n"
                        f"Log: {log.name} | {source_doctype}: {source_name}\n"
            ...
            frappe.logger("mobile_payments").info(
                f"Edahab instant success processed | log={log.name} | {source_doctype}={source_name}"
            )
    ...
    if log.status in ("Pending", "Initiated", "Failed") and log.provider_reference:
        try:
            ...
            if invoice_status == "Paid":
                ...
                # Commit first, then process payment inline
                if log.sales_invoice and not log.payment_entry:
                ...
                elif hasattr(log, "payment_entry") and log.payment_entry:
                    # PE-based token: finalize the existing Payment Entry
                    ...
                    try:
                        ...
                        frappe.log_error(
                            message=f"PE finalize failed for Edahab poll: {e}\n{frappe.get_traceback()}",
                            title="Edahab Poll PE Finalize Error",
                        )
...
def get_payment_link_status(token):
    ...
    log = frappe.db.get_value(
        "Mobile Payment Transaction Log",
        {"payment_link_token": token},
        ["name", "status", "sales_invoice", "payment_entry", "amount", "currency", "provider",
    ...
    elif log.get("payment_entry"):
        pe_docstatus = frappe.db.get_value("Payment Entry", log.payment_entry, "docstatus") or 0
        is_paid = pe_docstatus == 1
...
def refresh_hpp(token):
    ...
    if source_doctype == "Sales Invoice":
        ...
        amount = flt(pe_data.paid_amount) or flt(pe_data.received_amount)
        charge_currency = pe_data.paid_to_account_currency or pe_data.paid_from_account_currency or "USD"
    ...
    try:
        ...
        if result.get("success") and result.get("hpp_url"):
            ...
            frappe.logger("mobile_payments").info(
                ...
                f"{source_doctype}: {source_name} | Fresh HPP: {hpp_url[:60]}..."
            )
