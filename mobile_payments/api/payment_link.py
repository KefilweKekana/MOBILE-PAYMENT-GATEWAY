...
def _render_edahab_payment_page(token, invoice_id, amount, currency, prefill_phone=""):
    ...
    <style>
      ...
      /* Countdown ring */
      .ep-countdown-ring {{ position: relative; width: 100px; height: 100px; margin: 0 auto 20px; }}
      .ep-countdown-ring svg {{ width: 100%; height: 100%; transform: rotate(-90deg); }}
      .ep-ring-bg {{ fill: none; stroke: #e5e7eb; stroke-width: 6; }}
      .ep-ring-fg {{ fill: none; stroke: #10b981; stroke-width: 6; stroke-linecap: round; stroke-dasharray: 326.73; stroke-dashoffset: 0; transition: stroke-dashoffset 1s linear, stroke 0.3s; }}
      .ep-ring-fg.ep-ring-warn {{ stroke: #f59e0b; }}
      .ep-ring-fg.ep-ring-danger {{ stroke: #ef4444; }}
      .ep-countdown-text {{ position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 24px; font-weight: 700; color: #1f2937; }}
    </style>
    ...
    <div class="ep-page">
      <div class="ep-card">
          ...
          <div id="ep-processing-section" class="ep-center" style="display:none">
            <div class="ep-countdown-ring">
              <svg viewBox="0 0 120 120">
                <circle class="ep-ring-bg" cx="60" cy="60" r="52" />
                <circle class="ep-ring-fg" id="ep-ring" cx="60" cy="60" r="52" />
              </svg>
              <div class="ep-countdown-text" id="ep-countdown">2:00</div>
            </div>
            <h3 style="margin:0 0 8px; color:#1f2937;">Confirm on your phone</h3>
            <div class="ep-phone-prompt">
              <p><span class="ep-prompt-icon">&#128241;</span>
                 A USSD prompt has been sent to your phone.<br>
                 Please <strong>check your phone</strong> and <strong>confirm</strong> the payment.</p>
            </div>
            <p class="ep-status" id="ep-poll-status">Waiting for confirmation&hellip;</p>
          </div>
        ...
        <div class="ep-footer">
          <span style="color:#9ca3af; font-size:11px;">Powered by Edahab &middot; Mobile Payments</span>
        </div>
      </div>
    </div>
      ...
      initiate: function() {{
        ...
        this.api('mobile_payments.api.payment_link.edahab_initiate_payment', {{
          ...
          if (result.success) {{
            ...
            self.showSection('ep-processing-section');
            self.startPolling();
          }} else {{
      ...
      startPolling: function() {{
        ...
        this.countdownTimer = setInterval(function() {{
          ...
          if (self.secondsLeft <= 0) {{
            ...
            // Time's up — go back to phone entry
            self.reset();
            return;
          }}
          ...
          var statusEl = document.getElementById('ep-poll-status');
...
def edahab_payment_status(token):
    ...
    if log.status in ("Failed", "Cancelled", "Timeout"):
        return {
            "status": log.status,
            "error_message": log.error_message or f"Payment {log.status.lower()}.",
        }

    # ── Still pending — ask Edahab for the latest status ──
    if log.status in ("Pending", "Initiated") and log.provider_reference:
        try:
            from mobile_payments.api.edahab import EdahabClient
            client = EdahabClient()
            ...
            if invoice_status == "Paid":
                ...
                # Trigger payment processing (Payment Entry creation etc.)
