...
def pay(token):
    ...
    try:
        ...
        if result.get("success") and result.get("hpp_url"):
            ...
            # Build a retry link so the customer can try again easily
            retry_url = f"/api/method/mobile_payments.api.payment_link.pay?token={token}"
            frappe.respond_as_web_page(
                _("Payment Error"),
                _("Could not create a payment session: {0}<br><br>"
                  '<a href="{1}" style="display:inline-block;padding:12px 24px;'
                  'background:#10b981;color:#fff;border-radius:8px;'
                  'text-decoration:none;font-weight:600;margin-top:12px;">'
                  "Try Again</a><br><br>"
                  '<span style="color:#6b7280;font-size:13px;">'
                  "If this keeps happening, try USSD Push payment instead.</span>"
                  ).format(error_msg, retry_url),
