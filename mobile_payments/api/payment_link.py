...
def pay(token):
    ...
    inv_data = frappe.db.get_value(
        "Sales Invoice",
        invoice_id,
        ["outstanding_amount", "grand_total", "currency", "status", "docstatus"],
        as_dict=True,
    )

    # ── Check if the invoice (or its parent Sales Order) has been cancelled ──
    invoice_cancelled = (
        not inv_data
        or cint(inv_data.docstatus) == 2
        or (inv_data.status or "") == "Cancelled"
    )
    if not invoice_cancelled:
        # Also check if the originating Sales Order was cancelled
        linked_so = frappe.db.get_value(
            "Sales Invoice Item",
            {"parent": invoice_id, "docstatus": 1},
            "sales_order",
        )
        if linked_so and frappe.db.get_value("Sales Order", linked_so, "docstatus") == 2:
            invoice_cancelled = True

    if invoice_cancelled:
        # Clean up the token so future visits are fast-rejected
        if log.status not in ("Cancelled", "Completed", "Failed"):
            log.db_set("status", "Cancelled")
        log.db_set("payment_link_token", None)
        frappe.db.commit()
        frappe.respond_as_web_page(
            _("Order Cancelled"),
            _("This order has been cancelled and the payment link is no longer valid. "
    ...
    for entry in active_logs:
        try:
            ...
            if invoice_is_paid:
                ...
                new_status = "Cancelled" if invoice_is_cancelled else "Completed"
                frappe.db.set_value(
                    "Mobile Payment Transaction Log",
                    entry.name,
                    {"status": new_status, "payment_link_token": None},
                    update_modified=False,
                )
                continue
