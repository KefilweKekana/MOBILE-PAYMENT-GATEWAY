...
def pay(token):
    ...
    if src_docstatus == 2:
        # Ensure the token is wiped so repeat visits are fast-rejected
    ...
    if source_doctype == "Sales Invoice":
        ...
        # Check mobile_payment_status only if the custom field exists
        pe_mobile_status = ""
        ...
        if pe_data.docstatus == 1 or pe_mobile_status == "Completed":
            if log.status != "Completed":
                log.db_set("status", "Completed")
            log.db_set("payment_link_token", None)
            frappe.db.commit()
    ...
    ref_filter = {"sales_invoice": source_name} if source_doctype == "Sales Invoice" else {"payment_entry": source_name}
    ref_filter.update({
        "provider": provider,
        "hpp_url": ["!=", ""],
        "name": ["!=", log.name],
    ...
    if provider == "Edahab":
        ...
        if not prefill_phone or prefill_phone == "HPP" or prefill_phone == "0000000":
            customer = None
            if source_doctype == "Sales Invoice":
                customer = frappe.db.get_value("Sales Invoice", source_name, "customer")
            elif source_doctype == "Payment Entry":
                pt, pn = frappe.db.get_value("Payment Entry", source_name, ["party_type", "party"]) or (None, None)
                if pt == "Customer":
                    customer = pn
            if customer:
                try:
        ...
        frappe.logger("mobile_payments").info(
            f"Edahab payment link visited: {token[:8]}... | Doc: {source_name} | "
    ...
    try:
        ...
        if result.get("success") and result.get("hpp_url"):
            ...
            frappe.logger("mobile_payments").info(
                f"Payment link visited: {token[:8]}... | Doc: {source_name} | "
...
def edahab_initiate_payment(token, phone):
    ...
    if source_doctype == "Sales Invoice":
        ...
        charge_amount = _get_charge_amount(source_name) or float(inv_data.grand_total or 0)
        charge_currency = log.currency or inv_data.currency or "USD"
    else:
        pe_data = frappe.db.get_value(
        ...
        charge_amount = flt(pe_data.paid_amount) or flt(pe_data.received_amount)
        charge_currency = log.currency or pe_data.paid_to_account_currency or pe_data.paid_from_account_currency or "USD"

    # ── Initiate USSD Push via Edahab Purchase API ──
    # For SI, recalculate from outstanding; for PE, use already-computed charge_amount
    if source_doctype == "Sales Invoice":
        charge_amount = _get_charge_amount(source_name) or charge_amount
    ...
    try:
        ...
        frappe.logger("mobile_payments").info(
            ...
            f"Doc: {source_name} | Phone: {phone} | Result: {result.get('pending', False)}"
        )

        # If Edahab returned instant success, process it.
        if result.get("success") and source_doctype == "Sales Invoice":
            frappe.db.commit()
            ...
            try:
                ...
                frappe.log_error(
                    message=(
                        ...
                        f"Log: {log.name} | Doc: {source_name}\n"
            ...
            frappe.logger("mobile_payments").info(
                f"Edahab instant success PE processed | log={log.name} | doc={source_name}"
            )
