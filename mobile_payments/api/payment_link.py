...
def create_payment_link(invoice_id, provider=None, method=None, expiry_hours=24,
    ...
    # Calculate expiry
    expiry_hours = cint(expiry_hours) or 24
    link_expiry = None
    if expiry_hours > 0:
...
def pay(token):
    ...
    outstanding = float(inv_data.outstanding_amount or 0)
    ...
    try:
        ...
        if result.get("success") and result.get("hpp_url"):
            ...
            # Update the Sales Invoice payment link field with the fresh HPP URL
            try:
                link_field = "waafi_payment_link" if provider == "WaafiPay" else "edahab_payment_link"
                frappe.db.set_value("Sales Invoice", invoice_id, link_field, hpp_url, update_modified=False)
                frappe.db.commit()
            except Exception:
                pass
