...
def pay(token):
    ...
    # Block if the Sales Invoice has been cancelled (docstatus == 2)
    inv_docstatus = frappe.db.get_value("Sales Invoice", invoice_id, "docstatus")
    if inv_docstatus == 2:
        ...
        frappe.respond_as_web_page(
            _("Invoice Cancelled"),
            _("This invoice has been cancelled. The payment link is no longer valid."),
            http_status_code=410,
            indicator_color="red",
        )
        return
    ...
    # Charge only the outstanding portion (in the invoice's own currency)
