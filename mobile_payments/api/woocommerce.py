...
def woocommerce_payment_webhook():
    ...
    try:
        ...
        frappe.log_error(
            message=f"WooCommerce webhook error: {str(e)}\n{frappe.get_traceback()}",
            title="WC Webhook Error",
        )
...
def initiate_wc_payment():
    ...
    try:
        ...
        try:
            ...
            frappe.log_error(
                message=f"WC payment error: {str(e)}\n{frappe.get_traceback()}",
                title="WC Payment Error",
            )
        ...
        frappe.log_error(
            message=f"WC initiate payment error: {str(e)}\n{frappe.get_traceback()}",
            title="WC Initiate Payment Error",
        )
        frappe.local.response["http_status_code"] = 500
...
def check_wc_payment_status():
    ...
    if log.status in ("Pending", "Initiated"):
        try:
            ...
            frappe.log_error(
                message=f"WC status check error: {str(e)}",
                title="WC Status Check Error",
            )
